...
window.onload = function(){
	var id ="${param.id}";
	var value = parent.document.getElementById(id).value; //40288ce1596bfb5d01596c6324c90023,40288ce1596743aa015968481bfc004c
	var clientId ="${param.clientId}";
	if(value){
		var url='<%=request.getContextPath()%>/select/SelectAction_getUserNames.action?userIds='+value;
		Matrix.sendRequest(url,null,function(data){
			var callbackStr = data.data;
		    var jsonObj = JSON.parse(callbackStr);
		    MDataGrid005.setData(jsonObj);
		});
	}
}
	...
	function comfirmSelect(){
		...
		if(selectedItems!=null && selectedItems.size()>0){
				for(var i=0;i<selectedItems.length;i++){
			       userIds +=selectedItems[i].userId;
				   userNames +=selectedItems[i].userName;
					...
					<!-- 标签页  机构/人员/角色 -->
						<td id="td103" name="td103" style="width:35%;">
							<div class="main" style="width:100%;height:100%;">
    							<iframe id="main_iframe" src="tabPage.jsp?selectType=multi" style="width:100%;height:100%;" frameborder="0"></iframe>
    						</div>
						</td>
...
<tr><td colspan="2" style="border-style:none;width:100%;margin:0px;padding:0px;height:94%"><div id="DataGrid005_div" class="matrixComponentDiv" style="width:100%;height:100%;">
					...
					isc.MatrixListGrid.create({
						...
						exportCells:[{id:'userNo',title:'工号'},{id:'userName',title:'姓名'}],
						...
						{title:"工号",name:"userNo",type:'text',editorType:'InputHidden'},
