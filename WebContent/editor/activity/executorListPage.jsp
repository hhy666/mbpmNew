...
<head>
	...
	<script type="text/javascript">
		...
		function forwardFrame(row, element){
			...
			var src = "<%=request.getContextPath()%>/editor/editor_getExecutorDetailInfo.action?index="+index+"&activityId="+activityId+"&containerId="+containerId+"&include=true"
		...
		function addNewExecutor(){
			...
			var url = "<%=request.getContextPath()%>/editor/editor_addNewExecutor.action";
			Matrix.sendRequest(url,{'activityId':activityId,'i':listSize,'assignType':'baohan','containerId':containerId},function(data){
				if(data.data!=null){
					...
					var src = "<%=request.getContextPath()%>/editor/editor_getExecutorDetailInfo.action?index="+listSize+"&activityId="+activityId+"&containerId="+containerId+"&include=true"
					document.getElementById('iframeContent').src = src;
			...
			if(select!=null && select.length>0){
				layer.confirm("确认删除？", {
				   ...
				   closeBtn:0
					...
					var activityId = Matrix.getFormItemValue("activityId");
					var url = "<%=request.getContextPath()%>"+"/editor/editor_deleteExecutor.action";
					Matrix.send('form0',{'activityId':activityId,'index':index,'include':true},function(data){
						if(data.data!=null){
  ...
  <div id="box" style="height:100%;">
   	 <div id="top" style="height: 40%;width:100%;">
   	  	<div style="height: 30px;width: 100%;background: #F8F8F8;">
   	  		<button type="button" class="btn  btn-default toolBarItem" id="MtoolBarItemAdd" style="padding-left:5px;padding-right:5px;margin:0px;border:0;" onclick="addNewExecutor();">
				<img src="<%=path%>/resource/images/new.png" style="padding-right: 2px;">添加
			</button>
			<button type="button" class="btn  btn-default toolBarItem" id="MtoolBarItemDel" style="padding-left:5px;padding-right:5px;margin:0px;border:0;" onclick="delExecutor();">
   	 	...
   	 	<div style="height: calc(100% - 30px);width: 100%;overflow: auto">
   	 	   <form id="form0" name="form0" eventProxy="Mform0" method="post" action="<%=request.getContextPath()%>/editor/editor_deleteExecutor.action"
				...
				<script>
					$(document).ready(function() {   
						$("#DataGrid001_table").bootstrapTable({           
							...
							url: "<%=request.getContextPath()%>/editor/editor_getActivityExecutorInfo.action", 
						    ...
						    columns: [       //列配置项							 
							    ...
							    {title:"主键",field:"id",sortable:true,clickToSelect:true,editorType:'Text',type:'text',visible:false},
						        {title:"执行人员",field:"name",sortable:true,clickToSelect:true,editorType:'Text',type:'text'}
						    ],
