...
<head>
	...
	<script type="text/javascript">
		...
		function forwardFrame(row, element){
			...
			var src = "<%=request.getContextPath()%>/editor/editor_getExecutorDetailInfo.action?index="+index+"&activityId="+activityId+"&containerId="+containerId+"&include=false";
		...
		function addNewExecutor(){
			...
			Matrix.sendRequest(url,{'activityId':activityId,'i':listSize,'containerId':containerId},function(data){
				if(data.data!=null){
					...
					var src = "<%=request.getContextPath()%>/editor/editor_getExecutorDetailInfo.action?index="+listSize+"&activityId="+activityId+"&containerId="+containerId+"&include=false";
					document.getElementById('iframeContent').src = src;
			...
			if(select!=null && select.length>0){
				layer.confirm("确认删除？", {
				   ...
				   closeBtn:0
					...
					Matrix.send('form0',{'activityId':activityId,'index':index,'include':false},function(data){
						if(data.data!=null){
				...
				<script>
					$(document).ready(function() {   
						$("#DataGrid001_table").bootstrapTable({           
							...
							url: "<%=request.getContextPath()%>/editor/editor_getActivityNotExecutorInfo.action", 
						    ...
						    columns: [       //列配置项							 
						        ...
						        {title:"排除人员",field:"name",sortable:true,clickToSelect:true,editorType:'Text',type:'text'}
						    ],
