...
<head>
		...
		function comfirmSelect(){
			...
			//不能选择根节点的情况
			if(selectType=='1'){
				if(select!=null&&select.length==1&&select[0].type==2){
						Matrix.closeWindow(select[0]);
				}else{
					Matrix.warn("请选择部门");
				}
...
<script>
 var Mform0=isc.MatrixForm.create({
 				...
 				name:"Mform0",
 				position:"absolute",
 				action:"<%=request.getContextPath()%>/editor/editor_loadOrgAndUserTree.action?isLoadUser=false",
...
<form id="form0" name="form0" eventProxy="Mform0" method="post"
	...
	<tr id="tr001">
	<td id="td001" class="tdLayout" style="width:100%;height:90%;">
