	...
	<script type="text/javascript">
		...
		function saveData(){
			...
			if(editFlag==null || editFlag!='edit'){
				...
				var json = "{'componentId':'"+authId+"','component':'"+component+"','name':'"+name+"','containerId':'"+containerId+"','txType':'"+txType+"','entityId':'"+entityId+"','activityId':'"+activityId+"','condition':'"+condition+"'}";
				var synJson = isc.JSON.decode(json);
				//保存辅助事件的属性
				var url = "<%=request.getContextPath()%>/editor/process_saveAssistEventDetailProperty.action";
		...
		function openpopupSelectDialog1(){
			...
			parent.parent.parent.openNoComponent(this);   //editFlowProMainPage.jsp弹出
		}
		...
		function openpopupSelectDialog2(){
			...
			Matrix.setFormItemValue('editFlag','');
			parent.parent.parent.openCondiation(conditionValue,this);   //editFlowProMainPage.jsp弹出
		}
	</script>
</head>
<body>
...
<div id="j_id1" jsId="j_id1" style="position: relative; _layout: flowlayout; width: 100%; height: 100%">
 <form id="form0" name="form0" eventProxy="Mform0" method="post" action="<%=request.getContextPath()%>/editor/editor_.action"
	...
	<input type="hidden" name="express" id="express" value="${assistEvent.preCondition.value }"/>
