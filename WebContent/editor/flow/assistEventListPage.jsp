...
<script type="text/javascript">
	...
	function forwardFrame(row, element){
		...
		var src = "<%=request.getContextPath()%>/editor/flow/assistEventTabPage.jsp?entityId="+id+"&parentNodeId="+parentNodeId+"&index="+index+"&processdid="+processdid;
	...
	function addNewAssistEvent(){
		...
		var processdid = Matrix.getFormItemValue("processdid");
		var parentNodeId = Matrix.getFormItemValue("parentNodeId");
		var url = "<%=request.getContextPath()%>"+"/editor/process_addNewAssistEvent.action";
		Matrix.sendRequest(url,{'processdid':processdid,'parentNodeId':parentNodeId,'i':listSize},function(data){
			if(data.data!=null){
				...
				var id = newData.id;
				var src = "<%=request.getContextPath()%>/editor/flow/assistEventTabPage.jsp?entityId="+id+"&parentNodeId="+parentNodeId+"&index="+listSize+"&processdid="+processdid;
				document.getElementById('iframeContent').src = src;
	...
	function delAssistEvent(){
		...
		var id = record.id;
		var processdid = Matrix.getFormItemValue("processdid");
		var url = "<%=request.getContextPath()%>"+"/editor/process_deleteAssistEventById.action";
		Matrix.sendRequest(url,{'processdid':processdid,'assistEventId':id},function(data){
			if(data.data!=null){
					...
					<script>
						$(document).ready(function() {   
							$("#DataGrid001_table").bootstrapTable({           
								...
								url: "<%=request.getContextPath()%>/editor/process_getAssistEventsById.action", 
