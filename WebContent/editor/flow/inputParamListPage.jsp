		...
		if(record!=null){
		    ...
		    if(record.type == 1){       //流程变量
				...
				if(flowType == 0){
					...
					url = "<%=request.getContextPath()%>/editor/editor_getValueByStructId.action?type=1&activityId='+activityId+'";
				}else{
					url = "<%=request.getContextPath()%>/editor/process_getValueByStructId.action?type=1";
				}
					...
					<tr>
						<td colspan="2" style="border-style:none;width:100%;margin:0px;padding:0px;vertical-align: top;">
							...
							$(document).ready(function() {   
								...
								if(flowType == 0){
									var activityId = $('#activityId').val(); 
									url = "<%=path%>/editor/editor_getInputParameter.action?activityId='+activityId+'";
								}else{
									url = "<%=path%>/editor/process_getInputParameter.action";
								}
