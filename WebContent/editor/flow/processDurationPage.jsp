...
<head>
	...
	<script type="text/javascript">
		...
		window.onblur = function(){
			...
			if(synJson!=null){
				var url = "<%=request.getContextPath()%>"+"/editor/process_saveProcessDuration.action";
		...
		window.onload=function(){
			...
			if(phase=='4'){
				...
				document.getElementById('tr309').style.display='none';
				document.getElementById('tr310').style.display='none';
				document.getElementById('tr311').style.display='none';
				document.getElementById('tr320').style.display='none';
			}
			$('#date_checkBox301').val("${process.deadLineInfo.isSetCalendar=='true'?'1':''}");
			$('#popupSelectDialog301').val('${process.deadLineInfo.calendarId}');
		...
		function openpopupSelectDialog302(){
			parent.openVariables4(this);     //editFlowProMainPage.jsp弹出
		}
		
		//打开动态变量值窗口
		function openpopupSelectDialog303(){
			parent.openVariables5(this);     //editFlowProMainPage.jsp弹出
		}
		
		//打开业务日历弹出窗口
		function openpopupSelectDialog301(){
			parent.openCalendar(this);       //editFlowProMainPage.jsp弹出
		}
...
<form id="form0" name="form0" eventProxy="Mform0" method="post" action=""style="margin: 0px; height: 100%;overflow:auto;overflow-x:hidden;" enctype="application/x-www-form-urlencoded">
	...
	<table id="table301" class="tableLayout" style="width:100%;">
		<tr id="tr107" name="tr107">
			<td id="td107" name="td107"  style="width:100%;"><font id="font2">流程时限</font></td>
		</tr>
		<tr id="tr301">
		...
		<tr id="tr302">
			<td id="td312" class="tdValueCls" style="width:100%;text-align:left;">
				...
				<div id="comboBox301_div" style="display: inline-table;vertical-align: middle;width:10%;">
					<select id="comboBox301" name="comboBox301" value="${process.deadLineInfo.unitType=='HOUR'?'2':process.deadLineInfo.unitType=='MINUTE'?'3':'1'}" class="form-control" style="height:100%;">            
                       <option value="1" ${process.deadLineInfo.unitType == 'DAY' ? "selected" : ""}>天</option>
                       <option value="2" ${process.deadLineInfo.unitType == 'HOUR' ? "selected" : ""}>小时</option>
                       <option value="3" ${process.deadLineInfo.unitType == 'MINUTE' ? "selected" : ""}>分</option>
                    </select>
				</div>		
			</td>
		</tr>
		...
		<tr id="tr304">
			<td id="td332" class="tdValueCls" style="width:100%;text-align:left;">
				<div id="radio301_div" style="display: inline-table;margin-left: 20px;" >
					<input type="radio" class="box" name="time" id="time_0" value="1" ${process.deadLineInfo.deadLineType=='TimePoint'?'':'checked'}/>
				</div>
		...
		<tr id="tr305">
			<td id="td343" class="tdValueCls" style="width:100%;text-align:left;">
				<div style="display: inline-table;margin-left: 40px;" name="checkBox301_div" id="checkBox301_div" >
					<input type="checkbox" class="box" name="date_checkBox301" id="date_checkBox301" value="${process.deadLineInfo.isSetCalendar=='true'?'1':''}"  ${process.deadLineInfo.isSetCalendar=='true'?'checked':''}/>
				</div>
				<label name="label0032" id="label0032" class="control-label ">
					绑定业务日历
				</label>
		...
		<tr id="tr306" >
			<td id="td353" class="tdValueCls" style="width:100%;text-align:left;">
				...
				<div id="popupSelectDialog301_div" class="input-group" style="width:25%;">
					 <input type="text" id="popupSelectDialog301" name="popupSelectDialog301" value="${process.deadLineInfo.calendarId}"  class="form-control" readonly="true">
		...
		<tr id="tr307">
			<td id="td363" class="tdValueCls" style="width:100%;text-align:left;">
				<div style="display: inline-table;margin-left: 40px;" name="checkBox302_div" id="checkBox302_div" >
					<input type="checkbox" class="box" name="date_checkBox302" id="date_checkBox302" value="${process.deadLineInfo.isSetDeadLine=='true'?'1':''}" ${process.deadLineInfo.isSetDeadLine=='true'?'checked':''}/>
				</div>
				<label name="label0032" id="label0032" class="control-label ">
					设置时间值
				</label>	
		...
		<tr id="tr308">
			<td id="td373" class="tdValueCls" style="width:100%;text-align:left;">
				<div id="radio301_div"  style="display: inline-table;margin-left:60px;" >
					<input type="radio" class="box" name="value" id="value_0" value="1" ${process.deadLineInfo.valueType=='DYNAMIC'?'':'checked'}/>
				</div>
				<label name="label0032" id="label0032" class="control-label ">
					静态固定值
				</label>		
				<div id="input301_div" style="display: inline-table;vertical-align: middle;">
					<input id="input301" name="input301" type="text" value="${process.deadLineInfo.valueType=='DYNAMIC'?'':process.deadLineInfo.durationValue=='null'?'':process.deadLineInfo.durationValue}" class="form-control" style="height:100%;"/>
				</div>												
			</td>
		</tr>								
		<tr id="tr309">
			<td id="td383" class="tdValueCls" style="width:100%;text-align:left;">
				<div style="float:left;margin-left:60px;margin-right: 3px;line-height: 35px;">
					<div id="radio303_div" style="display: inline-table;" >
						<input type="radio" class="box" name="value" id="value_1" value="2" ${process.deadLineInfo.valueType=='DYNAMIC'?'checked':''}/>
					</div>
					<label name="label0032" id="label0032" class="control-label ">
						动态变量值
					</label>	
				...
				<div id="popupSelectDialog303_div" class="input-group" style="width:25%;">
					 <input type="text" id="popupSelectDialog303" name="popupSelectDialog303" value="${process.deadLineInfo.valueType=='STATIC'?'':process.deadLineInfo.durationValue=='null'?'':process.deadLineInfo.durationValue}"  class="form-control" readonly="true">
		...
		<tr id="tr310">
			<td id="td392" class="tdValueCls" style="width:100%;text-align:left;">
				<div id="radio304_div" style="display: inline-table;margin-left: 20px;" >
					<input type="radio" class="box" name="time" id="time_1" value="2" ${process.deadLineInfo.deadLineType=='TimePoint'?'checked':''}/>
				</div>
				<label name="label0032" id="label0032" class="control-label ">
					源自变量的时间点
				</label>			
		...
		<tr id="tr311">
			<td id="td303" class="tdValueCls" style="width:100%;text-align:left;">
				<div id="popupSelectDialog302_div" style="display: inline-table;margin-left: 40px;width:25%;">
					 <input type="text" id="popupSelectDialog302" name="popupSelectDialog302" value="${process.deadLineInfo.variableId}"  class="form-control" readonly="true">
		...
		<tr id="tr314">
			<td id="td332" class="tdValueCls" style="width:100%;text-align:left;">
				...
				<div id="comboBox302_div" style="display: inline-table;vertical-align: middle;width:15%;">
					<select id="comboBox302" name="comboBox302" value="${process.deadLineInfo.timeoutPolicy=='UPGRADEPRIORITY'?'1':process.deadLineInfo.timeoutPolicy=='SUBSTITUTE'?'2':process.deadLineInfo.timeoutPolicy=='COMPLETE'?'3':process.deadLineInfo.timeoutPolicy=='TERMINATE'?'4':'5'}" class="form-control" style="height:100%;">   			      
                       <option value="1" ${process.deadLineInfo.timeoutPolicy == 'UPGRADEPRIORITY' ? "selected" : ""}>提高优先级</option>
                       <option value="2" ${process.deadLineInfo.timeoutPolicy == 'SUBSTITUTE' ? "selected" : ""}>自动委托</option>
                       <option value="3" ${process.deadLineInfo.timeoutPolicy == 'COMPLETE' ? "selected" : ""}>自动完成</option>
                       <option value="4" ${process.deadLineInfo.timeoutPolicy == 'TERMINATE' ? "selected" : ""}>自动终止</option>
                       <option value="5" ${process.deadLineInfo.timeoutPolicy != 'UPGRADEPRIORITY' and process.deadLineInfo.timeoutPolicy != 'SUBSTITUTE' and process.deadLineInfo.timeoutPolicy != 'COMPLETE' and process.deadLineInfo.timeoutPolicy != 'TERMINATE' ? "selected" : ""}>无</option>
                    </select>
				</div>		
			</td>
		</tr>
