...
<head>
	...
	<script type="text/javascript">
		...
		function saveRemindEvent(){
			...
			if(synJson!=null){
				//双引号处理----asd
				if(synJson.popupSelectDialog001.indexOf("\"") >= 0 || synJson.popupSelectDialog001.indexOf("“") >= 0 || synJson.popupSelectDialog001.indexOf("”") >= 0 ){
					synJson.popupSelectDialog001 = synJson.popupSelectDialog001.replaceAll("\"","----#&#----");
				}
				var url = "<%=request.getContextPath()%>/editor/process_saveRemindEventDetailProperty.action";
		...
		function openpopupSelectDialog2(){
			parent.parent.parent.openChooseActivities(this);     //editFlowProMainPage.jsp弹出
		}
		
		//弹出条件编辑窗口
		function openpopupSelectDialog001(){
			parent.parent.parent.openConditionEdit(this);     //editFlowProMainPage.jsp弹出
		}
		
		//标题弹出选择流程变量窗口
		function openpopupSelectDialog1(){
			parent.parent.parent.openVariables6(this);     //editFlowProMainPage.jsp弹出
		}
		
		//内容弹出选择流程变量窗口
		function openpopupSelectDialog4(){
			parent.parent.parent.openVariables7(this);     //editFlowProMainPage.jsp弹出
		}
...
<form id="form0" name="form0" eventProxy="Mform0" method="post" action="<%=request.getContextPath()%>/editor/process_getCurProcessEditProperty.action"
	...
	<input type="hidden" name="express" id="express" value="${notifyInfo.preCondition }"/>
	
	<table id="table001" class="tableLayout" style="width:100%;" >
		<tr id="tr001">
			<td id="td001" class="tdValueCls" style="width:20%;text-align:right;">
		...
		<tr id="tr003">
			<td id="td005" class="tdValueCls" style="width:20%;text-align:right;" colspan="1">
			...
			<td id="td006" class="tdValueCls" style="width:50%;" colspan="1">
				<div id="comboBox001_div" style="vertical-align: middle;">
					<select id="comboBox001" name="comboBox001" value="${notifyInfo.receipientType=='ProcessAdmins'?'2':notifyInfo.receipientType=='ProcessParticipants'?'3':notifyInfo.receipientType=='ParticipantsExceptOwner'?'4':notifyInfo.receipientType=='TaskExecutor'?'5':notifyInfo.receipientType=='PROCESSECECUTOR'?'12':'1'}" 
					class="form-control" style="height:100%;" onchange="checkIsSelectedUser();">
                       ...
                       <option value="4" ${notifyInfo.receipientType=='ParticipantsExceptOwner' ? "selected" : ""}>除实际执行人员以外的参与者</option>
                       <option value="5" ${notifyInfo.receipientType=='TaskExecutor' ? "selected" : ""}>选定活动执行人员</option>
                       <option value="12" ${notifyInfo.receipientType=='PROCESSECECUTOR' ? "selected" : ""}>流程当前执行人员</option>
                    </select>
				</div>
			</td>
		...
		<tr id="tr004">
			<td id="td007" class="tdValueCls" style="width:20%;text-align:right;">
				<label id="label004" name="label004" id="label004">
					条件：
				</label>
			</td>
			<td id="td008" class="tdValueCls" style="width:80%;" colspan="2">
				<div id="popupSelectDialog001_div" class="input-group"  style="width:40%;">
		...
		<tr id="tr101">
			<td id="td101" class="tdValueCls" style="width:20%;text-align:right;border-right:0px;" colspan="1">
				<label id="label105" name="label105" id="label105">
					发送方式：
				</label>
			</td>
			<td id="td103" class="tdValueCls" style="width:80%;border-left:0px;" colspan="2">
			</td>
		</tr>
		<tr id="tr102" >
		...
		<tr id="tr201">
			<td id="td201" class="tdValueCls" style="width:20%;text-align:right;border-right:0px;">
				<label id="label001" name="label001" id="label001">
					发送消息：
				</label>
			</td>
			<td id="td002" class="tdValueCls" style="width:80%;border-left:0px;" colspan="2">	
			</td>
		</tr>
		<tr id="tr006">
			<td id="td011" class="tdValueCls" style="width:20%;text-align:right;">
				<label id="label006" name="label006" id="label006">
					标题：
				</label>
			</td>
			<td id="td012" class="tdValueCls" style="width:80%;" colspan="2">
				<div id="popupSelectDialog1_div" class="input-group" style="width:40%;">
		...
		<tr id="tr012">
			<td id="td019" class="tdValueCls" style="width: 20%; text-align: right; border-right: 0px;">
		...
		<tr id="tr013">
			<td id="td021" class="tdValueCls" style="width: 20%; text-align: center; border-right: 0px;"></td>
				<td id="td022" class="tdValueCls" style="width: 80%; border-left: 0px;" colspan="2">
					<div id="popupSelectDialog4_div" class="input-group" style="width:40%;">
