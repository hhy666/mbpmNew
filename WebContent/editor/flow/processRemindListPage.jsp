...
<script type="text/javascript">
	...
	function forwardFrame(row, element){
		...
		var src = "<%=request.getContextPath()%>"+"/editor/flow/processRemindTabPage.jsp?entityId="+id+"&parentNodeId="+parentNodeId+"&index="+index;
	...
	function addNewRemindEvent(){
		...
		var url = "<%=request.getContextPath()%>"+"/editor/process_addNewRemindEvent.action";
		Matrix.sendRequest(url,{'parentNodeId':parentNodeId,'i':listSize},function(data){
			if(data.data!=null){
				...
				var id = newData.id;
				var src = "<%=request.getContextPath()%>"+"/editor/flow/processRemindTabPage.jsp?entityId="+id+"&parentNodeId="+parentNodeId+"&index="+listSize;
				document.getElementById('iframeContent').src = src;
	...
	function delRemindEvent(){
		...
		var id = record.id;
		var url = "<%=request.getContextPath()%>"+"/editor/process_deleteRemindEventById.action";
		Matrix.send('form0',{'remindEventId':id},function(data){
			if(data.data!=null){
	 ...
	 <div id="box" style="height:100%;">
	 	<div id="top" style="height: 40%;width:100%;">
	   	 	...
	   	 	<div style="height: calc(100% - 30px);width: 100%;overflow: auto">
	   	 	   <form id="form0" name="form0" eventProxy="Mform0" method="post" action="<%=request.getContextPath()%>/editor/process_deleteRemindEventById.action"
					...
					<script>
						$(document).ready(function() {   
							$("#DataGrid001_table").bootstrapTable({           
								...
								url: "<%=request.getContextPath()%>/editor/process_getProcessRemindById.action", 
							    ...
							    columns: [       //列配置项							 
							        ...
							        {title:"流程提醒",field:"name",sortable:true,clickToSelect:true,editorType:'Text',type:'text'}
							    ],
