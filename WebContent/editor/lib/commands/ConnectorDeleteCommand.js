        ...
        if(this.firstExecute){
            ...
            CONNECTOR_MANAGER.connectorRemoveById(this.connectorId, true);
             ...
             var url = WebContextPath+'/matrix.dflow';
				var param="mode=flow&componentType=transition&command=delete&pdid="+pdid+"&containerId="+containerId+"&containerType="+containerType+"&componentId="+this.connectorId;
				getUrlData(url,param,function(data){
					//var data = eval(data);
					if("false" == data){
		                 alert("删除失败,请稍候再试!");
					}else{
						//CONNECTOR_MANAGER.connectorRemove(connector);
					}
				  }	
				);
