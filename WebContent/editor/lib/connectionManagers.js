...
ConnectorManager.prototype = {
     ...
     */
    connectorCreate:function(startPoint,endPoint,type, newId, name){
		...
		var connector = new Connector(startPoint,endPoint,type, id, name);
	...
	/**delete relevant connections of activity */
	connectorsRemoveOfActivity : function(actId){
        //remove the connector
		var ids = [];
		var k=0;
        for(var i=0; i<this.connectors.length; i++){
            if(this.connectors[i].fromActId == actId || this.connectors[i].toActId == actId){
				ids[k] = this.connectors[i].id;
				++k;
            }//end if
        }//end for

        for(var i=0; i<ids.length; i++){
            this.connectorRemoveById(ids[i], true);
        }//end if
	},

    
	/**delete relevant connections of two activity */
	connectorsRemoveOfTwoActivity : function(startActId, endActId){
        //remove the connector
		var ids = [];
		var k=0;
        for(var i=0; i<this.connectors.length; i++){
            if(this.connectors[i].fromActId == startActId || this.connectors[i].toActId == endActId){
				ids[k] = this.connectors[i].id;
				++k;
            }//end if
        }//end for
