...
/**Currently selected figure type (for D&D)*/
var selectedFigureType = null
 ...
 **/
function createFigure(fFunction, thumbURL, type){
                    ...
                    if(selectedFigureId != -1){
                        ...
                        */
                        ...
                        if(selectedFigureId == null || selectedFigureId == -1){
						    ...
						    if(status > 0){  //can not delete stated activity
						    	alert("流转过活动不允许删除!");
						    	return;
						    }  
                        }
	        			...
	        			if(isCustomDesign=='true' && isReadOnly=='true'){		 
						...
						var senddata ="containerType="+containerType+"&containerId="+containerId+"&mode=custom&componentType=customAct&pdid="+pdid+"&command=delete&componentId="+selectedFigureId;
						if("custom" != mode){
							senddata ="containerType="+containerType+"&containerId="+containerId+"&mode=flows&componentType="+figure.name+"&pdid="+pdid+"&command=delete&componentId="+selectedFigureId;
						}
        ...
        case STATE_NONE:
            ...
            switch(selectedObject.type) {
                case 'Connector':
                    ...
                    var type = CONNECTOR_MANAGER.connectorGetById(selectedConnectorId).oType;
                    var id = CONNECTOR_MANAGER.connectorGetById(selectedConnectorId).id;
                    initProperties(type,id);
                ...
                case 'Figure':
                    ...
                    var type = STACK.figureGetById(selectedFigureId).name;
                    var id = STACK.figureGetById(selectedFigureId).id;
                    initProperties(type,id);
                ...
                case 'Container':
                    ...
                    var type = STACK.containerGetById(selectedContainerId).oType;
                    var id = STACK.containerGetById(selectedContainerId).id;
                    initProperties(type,id);
                    Log.info('onMouseDown() + STATE_NONE  - change to STATE_CONTAINER_SELECTED');
                    redraw = true;
                    break;
                default:
                	initProperties(selectedObject.type,selectedObject.id);
                    //DO NOTHING
                    break;
            }
            break;
        ...
        */
        ...
        case STATE_SELECTING_MULTIPLE:
            ...
            */
            ...
            if(selectedGroupId != -1){
                var selectedGroup = STACK.groupGetById(selectedGroupId);
                //destroy current group (if temporary)
                if(selectedGroup && !selectedGroup.permanent){
                    STACK.groupDestroy(selectedGroupId);
                }
            }
        ...
        case STATE_CONNECTOR_PICK_SECOND:
            ...
            setUpEditPanel(con);
		     ...
		     //if no end figure,remove connector
		    ...
		    if(fCpOverId==-1 && fOverId==-1){
               ...
               state = STATE_NONE;
				...
				//set to act id
				var figure = STACK.figureGetAsSecondFigureForConnector(con.id);
				con.toActId = figure.id;
				
	          var url = WebContextPath+'/matrix.dflow';
				var param="mode=flow&componentType=transition&command=create&pdid="+pdid+"&containerId="+containerId+"&containerType="+containerType+"&componentId="+con.id+"&fromDID="+con.fromActId+"&toDID="+con.toActId;
				getUrlData(url,param,function(data){
					...
					if("false" == data){
						...
						state = STATE_NONE;
						draw();
						alert("合并活动只允许添加一条流出转移!");
					}
				  }	
				);
						
				if(figure.name == "DataObject" || figure.name == "DataStore"){
					con.isArrow = false;
                } 
            } 
        ...
        case STATE_CONNECTOR_MOVE_POINT:
		    ...
		    if(fCpOverId==-1 && fOverId==-1){
			   CONNECTOR_MANAGER.connectorRemove(con);
				...
				var point = CONNECTOR_MANAGER.connectionPointGetById(selectedConnectionPointId);
				var parentId = point.parentId;
				var con = CONNECTOR_MANAGER.connectorGetById(parentId);

				var fPoint = CONNECTOR_MANAGER.connectionPointGetFirstForConnector(con.id);
				var sPoint = CONNECTOR_MANAGER.connectionPointGetSecondForConnector(con.id);
				//first or last?
				if(fPoint != null && fPoint.id == point.id){
					var figure = STACK.figureGetAsFirstFigureForConnector(con.id);
					con.fromActId = figure.id;
					
					if(figure.name == "DataObject" || figure.name == "DataStore"){
						con.isArrow = false;
					}else{
						//修改活动transition
						var senddata ="containerType="+containerType+"&containerId="+containerId+"&mode=flow&componentType=transition&pdid="+pdid+"&command=changeTarget&componentId="+con.id+"&fromActId="+con.fromActId;
                ...
                }else if(sPoint != null && sPoint.id == point.id){
					var figure = STACK.figureGetAsSecondFigureForConnector(con.id);
					con.toActId = figure.id;;
					if(figure.name == "DataObject" || figure.name == "DataStore"){
						con.isArrow = false;
						...
						//修改活动transition
						var senddata ="containerType="+containerType+"&containerId="+containerId+"&mode=flow&componentType=transition&pdid="+pdid+"&command=changeTarget&componentId="+con.id+"&toActId="+con.toActId;
        ...
        case STATE_CONNECTOR_SELECTED:
            ...
            if(CONNECTOR_MANAGER.connectorGetById(selectedConnectorId)){
                var type = CONNECTOR_MANAGER.connectorGetById(selectedConnectorId).oType;
                var id = CONNECTOR_MANAGER.connectorGetById(selectedConnectorId).id;
                initProperties(type,id);
            }
    ...
    switch(state){
            ...
            if(mousePressed){
                ...
                if(STACK.figureIsOver(x,y)){ //over a figure
                    ...
                    var cId = CONNECTOR_MANAGER.connectorGetByXY(x, y);
 					...
 					setTransitionName(connector.name);
 					displayTransitionName();
        ...
        case STATE_FIGURE_SELECTED:
            ...
            if(mousePressed){ // mouse is (at least was) pressed
                ...
                if(handle != null && handle.type != 'none'){ //We are over a Handle of selected Figure 
                  var fOverId = STACK.figureGetByXY(x,y);
			      var figure = STACK.figureGetById(fOverId);
				  //can not change activity's size
				  if(figure != null && figure.name != "HumanTask"){
                    canvas.style.cursor = handle.getCursor();
...
/*	if(selectedFigureId != null && selectedFigureId != -1)
    	editActivity();
    else if(cId != null && cId != -1){
    	editTransition(cId);
    }else if(containerId != null && containerId != -1){
    	editSwimline(containerId);	
    }else{
    	editActivity();
    }	
*/    
...
function connectorPickFirst(x, y, ev){
    ...
    var fCpOverId = CONNECTOR_MANAGER.connectionPointGetByXY(x, y, ConnectionPoint.TYPE_FIGURE); //find figure's CP
     ...
     //if no start figure,not create connector
     if(fCpOverId == -1 && fOverId==-1){
		state = STATE_NONE;
		return;
     }
...
function connectorPickSecond(x, y, ev){
    ...
    //Ó¾ï¿½2ï¿½lï¿½ï¿½
      var figure = STACK.figureGetById(fOverId);
	if(figure != null && figure.name == "Note"){
	    fOverId = -1;
    }
...
//auto link target connect point
function connectorPickSecond2(x, y, conId){
	selectedConnectorId = conId;
    Log.group("main: connectorPickSecond");
    ...
    //current connector
    var con = CONNECTOR_MANAGER.connectorGetById(conId); //it should be the last one
    var cps = CONNECTOR_MANAGER.connectionPointGetAllByParent(con.id);
...
function generateMoveMatrix(fig, x,y){
    ...
    var dy = y - lastMove[1];
				...
				if(fig.type == "HSwimline" ){
					dx = 0;
                }else if(fig.type == "VSwimline"){
					dy = 0;
                } 
...
Alert("test"+JSON.stringify(CONNECTOR_MANAGER));
...
//                var $diagram = {c:canvas.save(), s:STACK, m:CONNECTOR_MANAGER};
   var $diagram = {c:canvasProps, s:STACK, m:CONNECTOR_MANAGER, p:CONTAINER_MANAGER, v: DIAGRAMO.fileVersion };
   var $serializedDiagram = JSON.stringify($diagram);
   alert(JSON.stringify(CONNECTOR_MANAGER));
   alert(JSON.stringify(CONTAINER_MANAGER));
   alert($serializedDiagram);
...
//            break;
        ...
        case 'container':
            ...
            //creates a container
            var cmdContainerCreate = new ContainerCreateCommand(300, 300, "HSwimline");
...
function documentOnMouseMove(evt){
    ...
    switch(state){
        case STATE_FIGURE_CREATE:
			...
			if(selectedFigureType == "HumanTask" || selectedFigureType == "AutomaticAct" 
				|| selectedFigureType == "SubflowAct"  || selectedFigureType == "BlockAct"
				|| selectedFigureType == "DataObject"  || selectedFigureType == "DataStore"){
				draggingFigure.style.width = '70px';
				draggingFigure.style.height = '45px';
	            draggingFigure.style.left = (evt.pageX - 45) + 'px';
		        draggingFigure.style.top = (evt.pageY - 29) + 'px';
			}else if(selectedFigureType == "StartAct" || selectedFigureType == "StopAct"
			        || selectedFigureType == "ConditionAct"  || selectedFigureType == "SplitAct"  || selectedFigureType == "JoinAct"){
                ...
                if(selectedFigureType=="VSwimline"){
					draggingFigure.style.width = '200px';
					draggingFigure.style.height = '600px';
                }else{
					draggingFigure.style.width = '900px';
					draggingFigure.style.height = '150px';
                }
            ...
            draggingFigure.addEventListener('mouseup', function (ev){
                ...
                switch(state){                                
                    case STATE_FIGURE_CREATE:
						...
						//HSwimline
						 if(selectedFigureType == "HSwimline"){
							x = 600;
							var cmdContainerCreate = new ContainerCreateCommand(x, y, "HSwimline", true,null, null, newFigureId);
							cmdContainerCreate.execute();
							History.addUndo(cmdContainerCreate);
							...
							break;
						 }else if(selectedFigureType == "VSwimline"){
							y = 380;
							var cmdContainerCreate = new ContainerCreateCommand(x, y, "VSwimline",true,null,null, newFigureId);
							cmdContainerCreate.execute();
							History.addUndo(cmdContainerCreate);
                        ...
                        if(window.createFigureFunction){
							...
							//judge whether add figure by selectedFigure

                            var cmdCreateFig = new FigureCreateCommand(window.createFigureFunction, x, y, newFigureId);
                            var createdFig = cmdCreateFig.execute();
                            History.addUndo(cmdCreateFig);
        ...
        case STATE_FIGURE_CREATE:
            ...
            if(eClicked.id != 'a'){
                if(draggingFigure){
                    ...
                    state = STATE_NONE;
                    draggingFigure = null;
                	clearActivity(selectedFigureType, newFigureId);
                    //evt.stopPropagation();
                }
            }
            break;
...
function getUrlData(url,param,callback){
		showMask();
	     ...
	     function redrawDiagramFromServer(data){
			var data = eval('(' +data+ ')');
			...
			function loadDiagram(actInfos, transitions, swimlines){
				...
				STACK.figures = [];
				 ...
				 connector = null;
				 connectorType = '';
				 CONNECTOR_MANAGER = new ConnectorManager();
				 CONTAINER_MANAGER = new ContainerFigureManager();
				 
				 var flowInfo = {};
				 //var actInfos = [];
				 //var transitions = [];
				
				/*var actInfo = {id:"01",x:200,y:200,name:"äººå·¥æ´»å¨ä¸",type:"HumanTask"};
				 actInfos[0] = actInfo;
				 actInfo = {id:"02",x:300,y:300,name:"äººå·¥æ´»å¨ä¸",type:"HumanTask"};
				 actInfos[1] = actInfo;
				 actInfo = {id:"03",x:400,y:400,name:"äººå·¥æ´»å¨ä¸",type:"HumanTask"};
				 actInfos[2] = actInfo;
				 */
				 ...
				 if(swimlines !=null){
						 for(var i=0 ; i< swimlines.length; ++i){
						    var swimline = swimlines[i];
						    
						    var cmdContainerCreate = new ContainerCreateCommand(swimline.x, swimline.y, swimline.type, false, swimline.width, swimline.height,swimline.id,swimline.name);
							cmdContainerCreate.execute();
							History.addUndo(cmdContainerCreate);
						}
				 }
				 ...
				 for(var i=0 ; i< actInfos.length; ++i){
				    var actInfo = actInfos[i];
	 			    createFigure(window["figure_"+actInfo.type] ,"lib/sets/network/m_human_s.png", actInfo.type);
					var cmdCreateFig = new FigureCreateCommand(window.createFigureFunction, actInfo.x, actInfo.y,actInfo.id,actInfo.status);
					var newId = cmdCreateFig.execute();

					var figure = STACK.figureGetById(actInfo.id);
					figure.status = actInfo.status;
					figure.aiid = actInfo.aiid;
					figure.setText(actInfo.name);
				}
			   ...
			   draw();
				 ...
				 for(var i=0 ; i< transitions.length; ++i){
				    var transition = transitions[i];
	 			    connectTwoActs(transition.fromActId, transition.toActId, transition.id, transition.name, transition.points);
				}
...
/*	var docVar;
	...
	if(!maskDiv){
		...
		//maskDiv.style="pointer-events:none;background-color:#fffff;opacity: 0.25;width:100%;height:100%;"
