...
Polygon.prototype = {
    ...
    paint:function(context){
        ...
        if(this.points.length > 1){
            context.moveTo(this.points[0].x+0.5, this.points[0].y+0.5);
            for(var i=1; i<this.points.length; i++){
                context.lineTo(this.points[i].x+0.5, this.points[i].y+0.5)
            }
        }
        context.closePath();
...
Figure.prototype = {
    ...
    getBounds: function(){
        var points = [];
        for (var i = 0; i < this.primitives.length; i++) {
        	if( this.primitives[i] instanceof ImageFrame){
                var bounds = this.primitives[i].getBounds();
                points.push(new Point(bounds[0], bounds[1]));
                points.push(new Point(bounds[2], bounds[3]));
        	}
    ...
    getX: function(){
		return this.primitives[0].vector[0].x;
    },

    getY: function(){
		return this.primitives[0].vector[0].y;
    },

    changePosition:function(newX, newY){
