...
<head>
	...
	function  deleteBusyDayOfDates(){
		...
		if(recordList==null||recordList.length==0){
			...
			for(var i=0;i<recordListJson.length;i++){
					...
					busyDayOfDateId = record.busyDayOfDateId;
	    		    
	    		    if(deleteIds!=""){
	    		       deleteIds = deleteIds + ",";
	    		    }
					deleteIds = deleteIds +busyDayOfDateId;
			}
			
			//deleteIds
			if(deleteIds==""){
				isc.warn("请选择要删除的特殊工作时间！");
	    		return false;
			}
			
		    var url = "<%=request.getContextPath()%>/calendar/calendarAction_deleteBusinessCalendarDate.action";
	...
	function createBusydayOfDate(){
		var src = "<%=request.getContextPath()%>/calendar/calendarAction_loadSaveBusyDayOfDate.action?operationType=add";
		MDialog0.initSrc = src;
		MDialog0.setTitle("添加特殊工作时间");
	...
	function updateBusydayOfDate(busyDayOfDateId){
		if(busyDayOfDateId==null){
			return false;
		}
		
		var src = "<%=request.getContextPath()%>/calendar/calendarAction_getBusinessCalendarDate.action?busyDayOfDateId="+busyDayOfDateId+"&operationType=update";
		MDialog0.initSrc = src;
		MDialog0.setTitle("更新特殊工作时间");
	...
	function updateBusyDayOfDateByButton(){
		 ...
		 if(recordList==null||recordList.length==0){
			parent.isc.warn('请选择要更新的特殊工作时间！');
			return false;
		 }else if(recordList.length==1){
		 	updateBusydayOfDate(recordList[0].busyDayOfDateId);
		 
		 }else{
			 parent.isc.warn("每次只能更新一个特殊工作时间！");
		 }
	...
	function onDialog0Close(data, oType){
		...
		if(data!=null){
			...
			if(operationType=="add"){//添加
			    ...
			    url = "<%=request.getContextPath()%>/calendar/calendarAction_createBusinessCalendarDate.action";
			}else if(operationType=="update"){//更新
				var calendarId = data.calendarId;
				var busyDayOfDateId = data.busyDayOfDateId;
				
				if(busyDayOfDateId==null||busyDayOfDateId.trim().length==0){
				...
				url = "<%=request.getContextPath()%>/calendar/calendarAction_updateBusinessCalendarDate.action";
			...
			<script>
				...
				isc.MatrixListGrid.create({
						...
						recordDoubleClick:function(viewer, record, recordNum, field, fieldNum, value, rawValue) {
	                       updateBusydayOfDate(record.busyDayOfDateId);
	                     },
						fields:[{//行号
							title:"&nbsp;",
							name:"MRowNum",
						   ...
						   {title:'工作日编码',name:'busyDayOfDateId',hidden:true},
				...
				MDataGrid0.setData(${requestScope.busyDayOfDates});
