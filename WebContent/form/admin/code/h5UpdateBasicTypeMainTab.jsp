...
<script type="text/javascript">
		function tabPanel001Click(){
			document.getElementById('InfoIframe').src = '<%=path%>/code/code_getCodeBasicItem.action?parentUUID=${param.entityId}&type=${type}';
		}
		function tabPanel002Click(){
			document.getElementById('InfoIframe').src = '<%=path%>/code/code_loadUpdateCodePage.action?uuid=${param.entityId}&type=${type}';
		}
	</script>
</head>
<body>
	<form id="Form0" name="Form0" method="post"  action="" style="margin: 0px; height: 100%;" enctype="application/x-www-form-urlencoded">
		<div id="tabContainer0_div" class="matrixComponentDiv" style="width: 100%; height: 100%;; position: relative;">
			...
			<ul id="TabContainer001" class="nav nav-tabs disable" style="position: relative;height: 30px"/>
				<li>
					<a href="#TabPanel001" data-toggle="tab" onclick="tabPanel001Click()">基本类型项</a>
				</li>
				<li>
					<a href="#TabPanel002" data-toggle="tab" onclick="tabPanel002Click()">基本信息</a>
				</li>
			</ul>
			<iframe id="InfoIframe" style="width:100%;height:calc(100% - 35px);top:30px " frameborder="0" src='<%=path%>/code/code_getCodeBasicItem.action?parentUUID=${param.entityId}'>
