...
<script type="text/javascript">
	...
	function submitByButton(){
		...
		if(oType=="add"){//添加
			addData = {'id':id,'name':name,'isEnable':isEnable,'flag':flag};	
		}else if(oType=="update"){//更新
			addData = {'id':id,'name':name,'isEnable':isEnable,'uuid':uuid,'flag':flag};
		}
		...
		//parentUUID,type
		dataSend(validateJson, url,"POST",function(data){
			...
			if(dataStr!=null){
				...
				var reMsg = document.getElementById("idRepeatMsg");  //选项值提示
			    var nameSign = document.getElementById("nameSign");   //选项显示值提示
	...
	function init(){
		...
		if(dialogType == "update"){
			document.getElementById("dataFormSubmitAndGoOnButton_div").style.display="none";
		}
	}
...
<table id="j_id3" jsId="j_id3" class="maintain_form_content">
	...
	<tr id="j_id14" jsId="j_id14">
        ...
        <div id="dataFormSubmitAndGoOnButton_div" class="matrixInline matrixInlineIE"
			style="position: relative;; width: 100px;; height: 22px;">
			<script>
			isc.Button.create({
					ID:"MdataFormSubmitAndGoOnButton",
					name:"dataFormSubmitAndGoOnButton",
					
					title:"保存并继续  ",
					displayId:"dataFormSubmitAndGoOnButton_div",
					position:"absolute",
					top:0,left:0,width:"100%",height:"100%",
					icon:Matrix.getActionIcon("save"),
					showDisabledIcon:false,
			...
			MdataFormSubmitAndGoOnButton.click=function(){
					flag = true;
					Matrix.showMask();
					if(!MForm0.validate()){
					    Matrix.hideMask();
		            ...
		            Matrix.hideMask();
            };</script></div>  &nbsp    &nbsp &nbsp &nbsp &nbsp &nbsp 
