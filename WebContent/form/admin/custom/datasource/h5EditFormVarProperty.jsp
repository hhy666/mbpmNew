...
<script type="text/javascript">
	...
	function hideSelect(modelType){
		if(modelType=='4'||modelType=='8'||modelType=='9'||modelType=='27'||modelType=='28'||modelType=='29'){//数字类型/附件类型/流程意见/单选/复选
        	$("li.es-visible").each(function(){ //遍历全部option
 	...
 	function modelTypeChange(){
		...
		if(uiType=='4'){//数字框
 		...
 		if(uiType == '5' || uiType == '11' || uiType == '22' || uiType == '8' || uiType == '7' || uiType == '9'){
 			...
 			document.getElementById('tr007').style.display='none';
 			document.getElementById('selectItemsStr').value = '';
							...
							if(data.data!="error"){
								...
								Matrix.sendRequest(url,synJson,function(data){
									if(data!=null &&data.data!=''){
										...
										if(json.result){
											 var initModelType = $("#initModelType").val();
											var modelType = $("#modelType").val();
											if(modelType!=initModelType){
												//主表主键操作
												var comId = mid;
												//获取当前组件
												var mcom = parent.Matrix.getComponentById(comId);
	
												if(mcom){ //如果表单中已经存在改组件则删除老的，自动添加新的
													var indexValue = 0;
													if(mcom.mparent){//根据父组件获取子索引
														var length = mcom.mparent.mchildren.length
														if(length > 1){
														    for(var i=0; i<=length; ++i){
																var child = mcom.mparent.mchildren[i];
																if(child.id == comId){
																   indexValue = i;
																   break;
																}
													...
													//删除原有组件
													parent.isc.MH.removeComponent(mcom);
	
													//添加新组件
													debugger;
													var formvar = Matrix.getFormItemValue("parentNodeId");
													var name = $('#dataName').val();
													
													var mNewcom = {};
													mNewcom.componentType = json.editorType;   //?
													mNewcom.editorType = json.editorType;
													mNewcom.formvar = formvar;
													mNewcom.sid = comId;
													mNewcom.mparent = mcom.mparent;
													mNewcom.mtitle = name;
													mNewcom.componentIndex = indexValue;
													mNewcom.action = 'updateEditType';
													mNewcom.command =  "create";
													mNewcom.parentId = mcom.mparent.id;
													mNewcom.isNewCom = "true";
													
													parent.isc.MH.createComponentSubmit(mNewcom);
												}
											} 
...
<body>
	<input type="hidden" name="initModelType" id="initModelType" value="${property.uiType}"/>
	...
	<div id="contatiner" style="position: absolute;height: 100%;width: 100%;padding: 10px">
			...
			<table id="table001" style="width:100%;" name="table001" class="tableLayout">
				...
				<tr id="tr002" name="tr002">
					...
					<td id="td004" style="width:70%;" name="td004" class="tdValueCls">
						<input placeholder="请输入名称" autocomplete="off" class="form-control" value="${property.name}" required="false" id="dataName" autocomplete="off" style="width:100%" name="dataName"  hideSelect/>
					</td>
				</tr>
				<tr id="tr003" name="tr003">
					...
					<td id="td006" style="width:70%;" name="td006" class="tdValueCls">
						<select class="form-control select2-accessible" id="modelType" onchange="modelTypeChange();">
				...
				<tr id="tr005" name="tr005">
					...
					<td id="td010" style="width:70%;" name="td010" class="tdValueCls">
						<input type="number" class="form-control" value="${property.length}" required="false" id="length" autocomplete="off" style="width:100%" name="length"/>
				  	</td>
				</tr>
				...
				<tr id="tr007" name="tr007">
					...
					<td id="td014" style="width:70%;" name="td014" class="tdValueCls">
						<div class="col-md-12 input-group " style="display: inline-table;  vertical-align: middle; width:100%">
							...
							<span class="input-group-addon addon-udSelect udSelect " onclick="onselectItemsClick()"><i class="fa fa-search"></i></span>
						</div>
				  	</td>
				</tr>
				<script>
		...
		<div id="bottom" class="cmdLayout">
			<input id="button1" type="button" class="x-btn ok-btn" value="保存" onclick="saveData()">
			<input id="button2" type="button" class="x-btn ok-btn" value="保存并继续" onclick="saveAndContinue()">
