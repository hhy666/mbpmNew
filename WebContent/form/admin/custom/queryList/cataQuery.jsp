	...
	<head>
		...
		function openOutData(formFlag,mBizId,formId,flag){
			 layer.open({
					...
					area : [ '90%', '90%' ],
					content : "<%=request.getContextPath()%>/form/admin/custom/queryList/outData.jsp?formFlag="+formFlag+"&flag="+flag+"&mBizId="+mBizId+"&formId="+formId+"&iframewindowid=layer03"
			 });
		}
	...
	      if(selectedItems!=null && selectedItems!=""){
				...
				for(var i=0;i<selectedItems.length;i++){
					...
					if(resl[0]!='{param.formFlag}'){
						iframe.Matrix.setFormItemValue('flag',resl[0]);
					}
				}
					...
					value+="]"
		      		iframe.Matrix.setFormItemValue('showListName',data);
					iframe.Matrix.setFormItemValue('showListVal',value);
					parent.parent.showListName=[];
				}else{
					iframe.Matrix.setFormItemValue('showListName',"");
					iframe.Matrix.setFormItemValue('showListVal',"");
			 }  
	      }
	}
		//移动端弹出输出数据项窗口
		function openMobileOutData(formFlag,mBizId,formId,flag){
			 layer.open({
					...
					shadeClose: false, //开启遮罩关闭
					area : [ '90%', '90%' ],
					content : "<%=request.getContextPath()%>/form/admin/custom/queryList/mobileOutData.jsp?formFlag="+formFlag+"&flag="+flag+"&mBizId="+mBizId+"&formId="+formId+"&iframewindowid=layer08"
			 });
		}
		
		//弹出输出数据项窗口回调
		function onlayer08Close(selectedItems){
		  var iframe = document.getElementsByTagName("iframe")[0].contentWindow;
	      if(selectedItems!=null && selectedItems!=""){
				if(selectedItems!=true){
				var data="";
				...
				for(var i=0;i<selectedItems.length;i++){
					...
					value+="]"
		      		iframe.Matrix.setFormItemValue('showMobileListName',data);
					iframe.Matrix.setFormItemValue('showMobileListVal',value);
					parent.parent.showMobileListName=[];
				}else{
					iframe.Matrix.setFormItemValue('showMobileListName',"");
					iframe.Matrix.setFormItemValue('showMobileListVal',"");
			 }  
	      }
	}
		...
		function openSetSort(formFlag,mBizId,formId){
			 layer.open({
					...
					shadeClose: false, //开启遮罩关闭
					area : [ '70%', '90%' ],
					content : "<%=request.getContextPath()%>/form/admin/custom/queryList/sortSet.jsp?formFlag="+formFlag+"&mBizId="+mBizId+"&formId="+formId+"&iframewindowid=layer02"
			 });
		}
		...
		function onlayer02Close(selectedItems){
			...
			       if(selectedItems!=null && selectedItems!=""){
				...
				      	iframe.Matrix.setFormItemValue('sortName',data);
						iframe.Matrix.setFormItemValue('sortContent',value);
					}else{
						iframe.Matrix.setFormItemValue('sortName',"");
						iframe.Matrix.setFormItemValue('sortContent',"");
				    } 
		...
		//查询设置弹出系统查询条件
		function openQuerySysCondiation(formFlag,flag,formId,firstCondition){
			layer.open({
		    	id:'layer01',
				type : 2,
				...
				shadeClose: false, //开启遮罩关闭
				area : [ '70%', '80%' ],
				content : "<%=path%>/query/form_condition.action?iframewindowid=layer01&formFlag="+formFlag+"&flag="+flag+"&formId="+formId+"&firstCondition="+encodeURI(firstCondition)
			});			
		}
		...
		function onlayer01Close(data){
			...
			 if(data!=null && data!=""){
				var data=eval("("+data+")")
		      	iframe.Matrix.setFormItemValue('sysQuerCondition',data.conditionText);
				iframe.Matrix.setFormItemValue('sysQuerConditionVal',data.conditionVal);
		      }else if(typeof(data) == "undefined"){
			
			  }else{
				iframe.Matrix.setFormItemValue('sysQuerCondition',"");
				iframe.Matrix.setFormItemValue("sysQuerConditionVal","");
			 }
		}
		
		//弹出用户输入条件
		function openInputCondition(formFlag,flag,formId){
			layer.open({
			    id:'layer04',
				type : 2,
				...
				shadeClose: false, //开启遮罩关闭
				area : [ '70%', '90%' ],
				content : "<%=request.getContextPath()%>/form/admin/custom/queryList/term.jsp?formFlag="+formFlag+"&flag="+flag+"&formId="+formId+"&iframewindowid=layer04"
		 });
		}
		
		//弹出用户输入条件回调
		function onlayer04Close(selectedItems){
		   var iframe = document.getElementsByTagName("iframe")[0].contentWindow;
		   if(selectedItems!=null && selectedItems!=""){
				if(selectedItems!=true){
				var data="";
				...
				if(sign==""||sign==null||sign=='undefined'){
					...
					if(arr[0]!='${param.formFlag}'){
						iframe.Matrix.setFormItemValue('flag',arr[0]);
					}
				}
					if(i==selectedItems.length-1){
							data+=selectedItems[i].name;
		...
		      	iframe.Matrix.setFormItemValue('userInputCondition',data);
				iframe.Matrix.setFormItemValue('userInputConditionVal',value);
			}else{
				iframe.Matrix.setFormItemValue('userInputCondition',"");
				iframe.Matrix.setFormItemValue('userInputConditionVal',"");
		  	}  
	      }
		  parent.userInputCondition=[];
		}  
		...
		function onlayer06Close(data){
			...
			if(data!=null && data!=""){
				   var userNames = data.allNames;
				   ...
				   iframe.Matrix.setFormItemValue('authValue',userNames);
			...
			var url = "<%=request.getContextPath()%>/query/query_getUUID.action";
						    ...
						    if(callbackJson!=null){
						    	// ("HorizontalContainer001Panel2").setContentsURL(src);
						      	  var src = webContextPath+"/form/admin/custom/queryList/querySet.jsp?formFlag=${formFlag}&catalogId=${param.catalogId}&oType=add&formId=${param.formId}&uuid="+callbackJson.uuid;
...
          		 Matrix.warn('请选择一行数据！');}
             ...
             var src = webContextPath+"/query/query_getListById.action?formFlag=${formFlag}&oType=update&formId=${param.formId}&uuid="+uuid;
             Matrix.getMatrixComponentById("HorizontalContainer001Panel2").setContentsURL(src);}
    }
    //点击跳转
    function onClick(){
...
          		 Matrix.warn('请选择一行数据！');}
             ...
             var data2 = {'data':{'uuid':uuid}};
							var url = "<%=request.getContextPath()%>/query/query_delete.action";
...
          		 Matrix.warn('请选择一行数据！');}
             ...
             var data2 = {'data':{'uuid':uuid,'catalogId':'${param.catalogId}'}};
							var url = "<%=request.getContextPath()%>/query/query_copyData.action";
							dataSend(data2,url,"POST",function(data2){
						    var callbackStr = data2.data;
						    var callbackJson = isc.JSON.decode(callbackStr);
						    if(callbackJson!=null&&callbackJson.result==true){
						    	// parent.parent.Matrix.forceFreshTreeNode("Tree0", "${param.parentNodeId}");
							   	 	Matrix.refreshDataGridData('DataGrid001');
							   	//加载报错，暂不刷新
							 //  	 	var src = "<%=request.getContextPath()%>/query/query_getListById.action?uuid="+callbackJson.uuid+"&formId=${param.formId}";
							//					Matrix.getMatrixComponentById("HorizontalContainer001Panel2").setContentsURL(src);
    ...
    function moveUp(uuid,index){
		...
		var url = "<%=request.getContextPath()%>/query/query_moveUpBasicItem.action";
    ...
    function moveDown(uuid,index){
    	...
    	var url = "<%=request.getContextPath()%>/query/query_moveDownBasicItem.action";
	...
	<body>
		...
		<script> var Mform0=isc.MatrixForm.create({ID:"Mform0",
				name:"Mform0",
				position:"absolute",
				action:"<%=request.getContextPath()%>/query/query_loadDataGridData.action?catalogId=${param.catalogId}",
				canSelectText : true,
				fields : [ {
	...
	isc.HLayout.create( {
		...
		canSelectText : true
				...
				{title:"查询模板名称",matrixCellId:"queryName",name:"queryName",canEdit:false,editorType:'Text',type:'text'},
				{title:"最后修改时间",matrixCellId:"lastEdit",name:"lastEdit",width:"60%",canEdit:false,align:"left",editorType:'DateItem',type:'date',formatCellValue:function(value, record, rowNum, colNum,grid){return Matrix.formatter(value,'date','yyyy年MM月dd日 HH时mm分', record, rowNum, colNum,grid);}}
				],
				autoSaveEdits:false,
				isMLoaded:false,
				autoDraw:false,
				...
				exportCells:[{id:'queryName',title:'查询模板名称'}],
				showRecordComponents:true,showRecordComponentsByCell:true});
				isc.MatrixDataSource.create({ID:'MDataGrid001_custom_filter_ds',fields:[{title:"查询模板名称",name:"queryName",type:'text',editorType:'Text'}]});
