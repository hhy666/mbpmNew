...
<script type="text/javascript">
	...
	function getData(optionId,operation){
		...
		$.ajax({
			type:'post',
			url:'<%=path %>/query/query_getH5QueryList.action',
			data:json,
			dataType:'json',
			success:function(data){
				if(data.length == 0){
					...
					html += '<label style="font-weight: bold;font-size: 14px;color:rgb(22, 105, 171);">查询名称：</label>';
					html += '</div>';
	...
	//根据查询设置主键加载查询设置详细信息页面
	function getQueryDetail(uuid){
		var catalogId = $('#catalogId').val();
		var formId = $('#formId').val();
		var formFlag = formId + "EntityVar";
		if(uuid!=null && uuid!=''){
			var src = "<%=path %>/form/admin/custom/queryList/h5QueryTabPage.jsp?formFlag="+formFlag+"&catalogId="+catalogId+"&oType=update&formId="+formId+"&uuid="+uuid;
			document.getElementById('iframeContent').src = src;	
		}else{
			Matrix.warn('该查询设置不存在！');
		}
	}
	...
	function addQuery(){
		...
		Matrix.sendRequest(url,null,function(data){	  
			...
			if(callbackJson!=null){   	
			   ...
			   content += '<label style="font-weight: bold;font-size: 14px;color:rgb(22, 105, 171);">添加查询设置</label>';
			   content += '</div>';
			   $("#module_div").html(content);
			   
			   var uuid = callbackJson.uuid
			   var src = "<%=path %>/form/admin/custom/queryList/h5QueryTabPage.jsp?formFlag="+formFlag+"&catalogId="+catalogId+"&oType=add&formId="+formId+"&uuid="+uuid;
			   document.getElementById('iframeContent').src = src;	
			}else{
			   Matrix.warn('添加失败');
			}    
	...
	function copyQuery(){
		if($("#module").val()!=null){
			...
			var url = "<%=request.getContextPath()%>/query/query_copyQuery.action?formId="+formId;
		...
		if(uuid!=null && uuid!=''){
			layer.confirm("确认删除？", {
			   ...
			   closeBtn:0
				...
				var url = "<%=request.getContextPath()%>/query/query_deleteQuery.action";
				Matrix.sendRequest(url,synJson,function(data){
					var callbackJson = isc.JSON.decode(data.data);
					if(callbackJson!=null){   	
					    if(callbackJson!=null&&callbackJson.result==true){
	...
	function moveUpQuery(){
		...
		if(checkIndex>0){
			...
			var catalogId = $('#catalogId').val();
			var url = "<%=request.getContextPath()%>/query/query_moveUpQuery.action";
	...
	function moveDownQuery(){
		...
		if(checkIndex<optionSize-1){
			...
			var catalogId = $('#catalogId').val();
			var url = "<%=request.getContextPath()%>/query/query_moveDownQuery.action";
			    ...
			    <ul class="nav">
					<li class="dropdown">
		               ...
		               <ul class="dropdown-menu operation">
		                <li><a href="#" onclick="addQuery()">添加</a></li>
		                <li><a href="#" onclick="copyQuery()">复制</a></li>          
		                <li><a href="#" onclick="deleteQuery()">删除</a></li>	           
		                <li><a href="#" onclick="moveUpQuery()">上移</a></li>
		                <li><a href="#" onclick="moveDownQuery()">下移</a></li>
		               </ul>
		            </li>
	            </ul>
			</div>
		</div>
		<div id="bottom" style="height: calc(100% - 80px);width:85%;overflow::auto;margin:0px auto 0px auto;background:white;background:white;box-shadow: 0 2px 4px 0 rgba(0,0,0,.08);">
