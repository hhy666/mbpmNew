...
function forwardPageByNodeType(viewer, node, recordNum){
    ...
    switch(nodeType){
    	...
    	case 2://基本类型 - 链接到列表页面
    		data = [];
    		 forwardPage(node,"form/admin/custom/queryList/tabQuery.jsp?entityId="+node.entityId+"&parentNodeId=parentNode.id&catalogId="+uuid+"&formId="+formId);
    		 break;
    		 }
	}
	...
	function addCodeNode(target, type){	
		data = [];
		forwardPage(target,"query/code_loadAddCodePage.action?parentUUID="+target.entityId+"&parentNodeId=treeNode.id&oType=add&type="+type);
	}
	...
	function modifyCodeNode(target){	
	    ...
	    if(type==0){//根节点 更新后刷新自身
			forwardPage(target,"query/code_loadUpdateCodePage.action?entityId="+target.entityId+"&parentNodeId=treeNode.id&oType=update&type="+type);
	    }else if(type==1){//子目录
	    	forwardPage(target,"query/code_loadUpdateCodePage.action?entityId="+target.entityId+"&parentNodeId=parentNode.id&oType=update&type="+type);
	    }else if(type==2){//基本类型
	    	forwardPage(target,"query/code_loadUpdateCodePage.action?entityId="+target.entityId+"&parentNodeId=parentNode.id&oType=update&type=2");
	    }
	...
	function deleteCodeNode(target){
	 var nodeType = target.type;//节点类型
	    ...
	    switch(nodeType){
	    	...
	    	break;
	    	case 2:
	    	confirmMsg ="节点"+target.text;
	    	break;
	   }
	isc.confirm('确定要删除'+confirmMsg+'吗?',function(data){//true or null
		      if(data){
		      	 var url ="query/code_deleteCode.action?entityId=treeNode.entityId&parentNodeId=parentNode.id&type="+target.type;
...
var MForm0=isc.MatrixForm.create({
			...
			action:"<%=request.getContextPath()%>/query/code_loadDataGridData.action",
			fields:[{
...
isc.Menu.create({
	...
	data: [ 
			...
			{title:"添加查询",click:"addCodeNode(target, 2)",icon:Matrix.getActionIcon("add")},
		    {title:"刷新",click:"Matrix.refreshTreeNode(target)",icon:Matrix.getActionIcon("refresh")},
