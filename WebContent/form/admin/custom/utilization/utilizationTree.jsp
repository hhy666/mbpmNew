...
function forwardPageByNodeType(viewer, node, recordNum){
    ...
    switch(nodeType){
    	...
    	case 2://基本类型 - 链接到列表页面
    		data = [];
    		 forwardPage(node,"form/admin/custom/utilization/tabUtilization.jsp?entityId="+node.entityId+"&parentNodeId=parentNode.id&catalogId="+uuid+"&formId="+formId);
    		 break;
    		 }
	}
	...
	function modifyCodeNode(target){	
	    ...
	    if(type==0){//根节点 更新后刷新自身
			forwardPage(target,"utilization/code_loadUpdateCodePage.action?entityId="+target.entityId+"&parentNodeId=treeNode.id&oType=update&type="+type);
	    }else if(type==1){//子目录
	    	forwardPage(target,"utilization/code_loadUpdateCodePage.action?entityId="+target.entityId+"&parentNodeId=parentNode.id&oType=update&type="+type);
	    }else if(type==2){//基本类型
	    	forwardPage(target,"utilization/code_loadUpdateCodePage.action?entityId="+target.entityId+"&parentNodeId=parentNode.id&oType=update&type=2");
	    }
	...
	isc.confirm('确定要删除'+confirmMsg+'吗?',function(data){//true or null
		      if(data){
		      	 var url ="utilization/code_deleteCode.action?entityId=treeNode.entityId&parentNodeId=treeNode.parentNodeId&type="+target.type;
...
var MForm0=isc.MatrixForm.create({
			...
			action:"<%=request.getContextPath()%>/utilization/codeloadDataGridData.action",
			fields:[{
...
<form id="Form0" name="Form0" eventProxy="MForm0" method="post"
	action="<%=request.getContextPath()%>/utilization/code_loadDataGridData.action"
