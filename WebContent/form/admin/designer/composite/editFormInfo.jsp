...
<table style="cellpadding:0;cellspacing:0;width: 100%; height: 100%">
		...
		<td style="height: 60px;background-image:url(<%=request.getContextPath()%>/matrix/resource/images/probanner.jpg);">
			...
			%>
						--><a href="<%=request.getContextPath()%>/form/admin/designer/composite/editFormActions.jsp">操作按钮</a>-->
						<b>高级属性</b>
			<% 
			  }
			%>				
			...
			<div id="tabContainer0_div" class="matrixComponentDiv" 
				...
				<script> 
					var MtabContainer0 = isc.TabSet.create({
						...
						displayId:"tabContainer0_div",height: "100%",
						width: "100%",position: "relative",align: "center",
						tabBarPosition: "top",tabBarAlign: "left",
						showPaneContainerEdges: false,showTabPicker: true,
						showTabScroller: true,selectedTab: 4,
						tabs: [ 
								{title: "高级属性",
									pane:isc.HTMLPane.create({ID:"MtabContainer0Panel3",height: "100%",autoDraw: false,showEdges:false,contentsType:"page",
									contentsURL:"<%=request.getContextPath()%>/form.daction?componentType=EditForm&componentId=${param.componentId}&command=editPart&page=advance&mode=propertyedit",overflow:'auto'})
								}
						  	] 
						});
	...
	<tr >
		<td style="height:28px">
			...
			Matrix.showMask();
			document.getElementById('Form0').action="<%=request.getContextPath()%>/form/admin/designer/composite/editFormActions.jsp";
