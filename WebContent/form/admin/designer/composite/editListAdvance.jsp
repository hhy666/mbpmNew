...
<table style="cellpadding:0;cellspacing:0;width: 100%; height: 100%">
	<tr>
		...
		<br>
			&nbsp;&nbsp;&nbsp;导航:&nbsp;&nbsp;&nbsp;
			<a href="<%=request.getContextPath()%>/form/admin/designer/composite/editListBasic.jsp">基本信息</a>-->
			<a href="<%=request.getContextPath()%>/form/admin/designer/composite/editListDataFields.jsp">数据项</a>-->
			<a href="<%=request.getContextPath()%>/form/admin/designer/composite/editListQueryForm.jsp">查询排序</a>
<% 
if(MFormContext.getPhaseId() == 2 || MFormContext.getPhaseId() == 3){  //设计开发阶段再显示
%>
			--><a href="<%=request.getContextPath()%>/form/admin/designer/composite/editListActions.jsp">操作列表</a>-->
			<b>高级属性</b>
<% 
  }
%>
...
<div id="tabContainer0_div" class="matrixComponentDiv" style="width:100%;height:100%;position:relative;" >
	<script> var MtabContainer0 = isc.TabSet.create({ID:"MtabContainer0",overflow:'auto',displayId:"tabContainer0_div",height: "100%",width: "100%",position: "relative",align: "center",tabBarPosition: "top",tabBarAlign: "left",showPaneContainerEdges: false,showTabPicker: true,showTabScroller: true,selectedTab: 4,
		tabs: [ 
			...
			contentsURL:"<%=request.getContextPath()%>/form.daction?componentType=EditList&componentId=${param.componentId}&command=editPart&page=basic&mode=propertyedit"})},
			...
			contentsURL:"<%=request.getContextPath()%>/form.daction?componentType=EditList&componentId=${param.componentId}&command=editPart&page=advance&mode=propertyedit",overflow:'auto'})}
			  ] });
	...
	<tr>
		<td style="height:28px">
			...
			Matrix.showMask();
			document.getElementById('Form0').action="<%=request.getContextPath()%>/form/admin/designer/composite/editListQueryForm.jsp";
			document.getElementById('Form0').submit();
			Matrix.hideMask();
			}
			</script>
