...
<div id="tabContainer0_div" class="matrixComponentDiv" style="width:100%;height:100%;;position:relative;" >
	<script> var MtabContainer0 = isc.TabSet.create({ID:"MtabContainer0",overflow:'auto',displayId:"tabContainer0_div",height: "100%",width: "100%",position: "relative",align: "center",tabBarPosition: "top",tabBarAlign: "left",showPaneContainerEdges: false,showTabPicker: true,showTabScroller: true,selectedTab: 4,
		tabs: [ 
			...
			contentsURL:"<%=request.getContextPath()%>/form.daction?componentType=DataCell&command=editPart&level=subItem&page=basic&mode=propertyedit"})},
<%
MUIComponent parentObj = (MUIComponent)request.getSession().getAttribute(ActionConstants.COMPOSITE_MODEL);
if(parentObj != null &&parentObj instanceof QueryList){
...
%>
			...
			contentsURL:"<%=request.getContextPath()%>/form/admin/designer/h5DataValidatorList.jsp?componentType=DataCell"})},
<%
}
...
<%
	if(com.matrix.form.admin.common.utils.CommonUtil.getCurPhase() != CompTypeConstant.PHASE_CUSTOMIZE){ //非业务定制阶段才显示
%>
			{title: "显示类型",pane:isc.HTMLPane.create({ID:"MtabContainer0Panel3",height: "100%",overflow: "hidden",autoDraw: false,showEdges:false,contentsType:"page",
			contentsURL:"<%=request.getContextPath()%>/form.daction?componentType=DisplayType&command=list&level=subItem&mode=propertyedit"})},

			
			{title: "高级属性",pane:isc.HTMLPane.create({ID:"MtabContainer0Panel2",height: "100%",overflow: "hidden",autoDraw: false,showEdges:false,contentsType:"page",
			contentsURL:"<%=request.getContextPath()%>/form.daction?componentType=DataCell&command=editPart&level=subItem&page=advance&mode=propertyedit"})}
<%
	}
%>
			  ] });
