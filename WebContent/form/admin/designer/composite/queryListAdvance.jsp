...
<table style="cellpadding:0;cellspacing:0;width: 100%; height: 100%">
	<tr>
...
%>
			--><a href="<%=request.getContextPath()%>/form/admin/designer/composite/queryListActions.jsp">操作列表</a>-->
			<b>高级属性</b>
<% 
  }
%>
...
<div id="tabContainer0_div" class="matrixComponentDiv" style="width:100%;height:100%;;position:relative;" >
	<script> var MtabContainer0 = isc.TabSet.create({ID:"MtabContainer0",overflow:'auto',displayId:"tabContainer0_div",height: "100%",width: "100%",position: "relative",align: "center",tabBarPosition: "top",tabBarAlign: "left",showPaneContainerEdges: false,showTabPicker: true,showTabScroller: true,
		tabs: [ 
			...
			contentsURL:"<%=request.getContextPath()%>/form.daction?componentType=QueryList&componentId=${param.componentId}&command=editPart&page=basic&mode=propertyedit"})},
			{title: "合并头",pane:isc.HTMLPane.create({ID:"MtabContainer0Panel1",height: "100%",overflow: "hidden",autoDraw: false,showEdges:false,contentsType:"page",
			contentsURL:"<%=request.getContextPath()%>/form.daction?componentType=MergeHeader&componentId=${param.componentId}&command=list&level=subItem&mode=propertyedit"})},
			{title: "高级属性",pane:isc.HTMLPane.create({ID:"MtabContainer0Panel3",height: "100%",autoDraw: false,showEdges:false,contentsType:"page",
			contentsURL:"<%=request.getContextPath()%>/form.daction?componentType=QueryList&componentId=${param.componentId}&command=editPart&page=advance&mode=propertyedit",overflow:'auto'})}
			  ] });
	...
	<tr >
		<td style="height:28px">
			...
			Matrix.showMask();
			document.getElementById('Form0').action="<%=request.getContextPath()%>/form/admin/designer/composite/queryListActions.jsp";
		...
		<div id="ToolBar0_div" style="width: 100%; overflow: hidden;height:28px;">
		<script>isc.ToolStrip.create({ID:"MToolBar0",displayId:"ToolBar0_div",width: "100%",height: "28px",position: "relative",align: "right",members: [ MToolBarItem3,"separator",MToolBarItem4,"separator",MToolBarItem1,"separator",MToolBarItem2 ] });isc.Page.setEvent(isc.EH.RESIZE,"MToolBar0.resizeTo(0,0);MToolBar0.resizeTo('100%','100%');",null);
			MToolBarItem4.setDisabled(true);
		</script></div>
		</td>
	</tr>
</table>
