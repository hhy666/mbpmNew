...
<div id="tabContainer0_div" class="matrixComponentDiv" style="width:100%;height:100%;position:relative;" >
	...
	<script> var MtabContainer0 = isc.TabSet.create({ID:"MtabContainer0",displayId:"tabContainer0_div",layoutTopMargin:0,
	   height: "100%",width: "100%",position: "relative",align: "center",tabBarPosition: "top",tabBarAlign: "left",showPaneContainerEdges: false,showTabPicker: true,showTabScroller: true,
		...
		tabs: [ 
			...
			contentsURL:"<%=request.getContextPath()%>/form.daction?componentType=<%=componentType%>&componentId=${param.componentId}&command=editPart&page=basic&mode=propertyedit&level=subItem"})}
<%if("DynamicTreeNode".equals(componentType)){ %>
			,{title: "类型",pane:isc.HTMLPane.create({ID:"MtabContainer0Panel1",height: "100%",overflow: "hidden",margin:0,autoDraw: false,showEdges:false,contentsType:"page",
			contentsURL:"<%=request.getContextPath()%>/form.daction?componentType=DynamicTreeNodeType&level=subItem&command=list&mode=propertyedit"})}
<%}
   if(!"StaticTreeNode".equals(componentType)){ %>
			...
			contentsURL:"<%=request.getContextPath()%>/form.daction?componentId=${param.componentId}&command=editPart&level=subItem&page=advance&mode=propertyedit&componentType=<%=componentType%>"})}
			<% 
} 
if("TreeNode".equals(componentType)){
%>
<%
}
%>
