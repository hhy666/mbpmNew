...
<div id="tabContainer0_div" class="matrixComponentDiv" style="width:100%;" >
	<script> var MtabContainer0 = isc.TabSet.create({ID:"MtabContainer0",displayId:"tabContainer0_div",height: "100%",width: "100%",position: "relative",align: "center",tabBarPosition: "top",tabBarAlign: "left",showPaneContainerEdges: false,showTabPicker: true,showTabScroller: true,selectedTab: 4,
		...
		tabs: [ 
			...
			contentsURL:"<%=request.getContextPath()%>/form.daction?componentType=HorizontalContainer&command=editPart&page=edit&mode=propertyedit"})},
			{title: "分栏信息",pane:isc.HTMLPane.create({ID:"MtabContainer0Panel1",height: "100%",overflow: "hidden",autoDraw: false,showEdges:false,contentsType:"page",
			contentsURL:"<%=request.getContextPath()%>/form.daction?componentType=BorderPanel&command=list&level=subItem&mode=propertyedit"})}
			  ] });
