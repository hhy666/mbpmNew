...
<div id="tabContainer0_div" class="matrixComponentDiv" style="width:100%;" >
	<script> var MtabContainer0 = isc.TabSet.create({ID:"MtabContainer0",displayId:"tabContainer0_div",height: "100%",width: "100%",position: "relative",align: "center",tabBarPosition: "top",tabBarAlign: "left",showPaneContainerEdges: false,showTabPicker: true,showTabScroller: true,
		tabs: [ 
			...
			contentsURL:"<%=request.getContextPath()%>/form.daction?componentType=TabContainer&command=editPart&page=edit&mode=propertyedit"})},
			{title: "标签页",pane:isc.HTMLPane.create({ID:"MtabContainer0Panel1",height: "100%",overflow: "hidden",autoDraw: false,showEdges:false,contentsType:"page",
			contentsURL:"<%=request.getContextPath()%>/form.daction?componentType=TabPanel&command=list&level=subItem&mode=propertyedit"})}
			  ] });
...
isc.Page.setEvent(isc.EH.LOAD,
	...
	<tr id="j_id190" jsId="j_id190">
		<td id="j_id191" jsId="j_id191" style="height:28px;padding:0px;" 
			colspan="1" rowspan="1">
		...
		<div id="ToolBar0_div" style="width: 100%; height:28px;overflow: hidden;"><script>isc.ToolStrip.create({ID:"MToolBar0",displayId:"ToolBar0_div",width: "100%",height: "*",position: "relative",align: "right",members: [ MToolBarItem1,"separator",MToolBarItem2 ] });isc.Page.setEvent(isc.EH.RESIZE,"MToolBar0.resizeTo(0,0);MToolBar0.resizeTo('100%','100%');",null);</script></div>
		</td>
	</tr>
</table>
