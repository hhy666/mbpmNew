  ...
  /******** upmove grid list data  end*********/
		...
		function keyValidator() {
		    ...
		    if (data != null && dataLength > 0){
		        ...
		        if (count > 0) {
		            if (!isEntityCol) {
		                Matrix.warn("主键必须为实体字段!");
		                return false;
		            }
		            
		            if(!isRequiredMsg){
		           	    Matrix.warn("主键必须为必填字段!");
		                return false;
		            }
		            count = 0;
		            return true;
		        } else {
		            Matrix.warn("至少有一个字段为主键!");
		            return false;
		        }
		    } else { //属性为空不验证
		        return true;
...
<table id="dataTable" jsId="dataTable" cellpadding="0px" cellspacing="0px" style="width:100%;height:100%;">
		...
		<tr id="j_id7" jsId="j_id7">
	 		...
	 		var map = {'1':'文本框','2':'日期框','3':'时间框',
					  			...
					  			'11':'列表框','27':'单文件上传','28':'多文件上传','14':'富文本框',
			...
			if("<%=curPhase%>" != '4'){
				 map = {'1':'文本框','2':'日期框','3':'时间框',
					  			...
					  			'11':'列表框','27':'单文件上传定制','28':'多文件上传定制','14':'富文本框',
			...
			var displayFormatValueMap = {'':'','yyyy-MM-dd':'yyyy-MM-dd','yyyy年MM月dd日':'yyyy年MM月dd日','yyyy-MM-dd hh:mm':'yyyy-MM-dd hh:mm','yyyy年MM月dd日 hh:mm':'yyyy年MM月dd日 hh:mm'} ;
			var displayFormatVM = ['','yyyy-MM-dd','yyyy年MM月dd日','yyyy-MM-dd hh:mm','yyyy年MM月dd日 hh:mm'] ;
			
			var valueMapNum=[''];
			var displayValueMapNum={'':''}
...
//					  			var uiTypeValue = grid.getEditValue(rowNum,"uiType");
					...
					},
					  {
					  		...
					  		type:'text',
					  		showHover:false,
					  		valueMap:['1','2','3','4','5','22','6','7','8','9','10','30','11','27','28','14','20','21','23','24','25','26','29'],
					  		formatCellValue:function(value, record, rowNum, colNum,grid){
					  		...
					  		editorProperties:{//为13关联对象时不可用
						  		...
						  		editorProperties:{
						  			...
						  			'9':'checkBoxGroup',
						  			'10':'inputTextArea','11':'listBox','27':'fileUpload','14':'richText',
...
function onDataGrid0_optionsDialogClose(data) {
	...
	<script>
     ...
     function getEditAttributeId(){
		  ...
		  if(dataGrid){
		 	if(!dataGrid.getSelection() || dataGrid.getSelection().length==0){
				Matrix.warn("没有数据被选中，不能执行此操作。",{ width:150,height:70});
				return null;
			}
