	...
	<script> 
		var MtabContainer0 = isc.TabSet.create({
				...
				tabBarControls : [
				 ...
				 tabs: [ 
				 	{
				 	title: "版本列表",pane:isc.HTMLPane.create({
...
document.getElementById('tabContainer0Bar0_div').appendChild(document.getElementById('tabContainer0Bar0_div2'));
	...
	MtabContainer0Panel3.setContentsURL('<%=request.getContextPath()%>/form/formInfo_getFormBasicMsgPage.action?nodeUuid=${catalogInfo.uuid}&parentNodeId=${param.parentNodeId}');//nodeUuid传递参数
	MtabContainer0Panel0.setContentsURL('<%=request.getContextPath()%>/form/formInfo_loadFormVersionsPage.action?nodeUuid=${catalogInfo.uuid}');//nodeUuid接收参数
