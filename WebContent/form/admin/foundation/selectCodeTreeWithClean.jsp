...
<script type="text/javascript">
     ...
     function cleanByButton(){
     	var cleanItem = '{"id":"_cleanSelectItem_"}';
     	Matrix.closeWindow(cleanItem);
     }
     //双击节点选择变量 自定义函数
     function nodeDoubleClickCustomFun(record){
        var type = record.type;
     	if(type==2||type==3){
     	    ...
     	    isc.say('请选择数据项!',{ width:150,height:70});
     	}
     }
</script>
</head>
...
<script> 
	var MForm0=isc.MatrixForm.create({
				...
				name:"MForm0",
				position:"absolute",
				action:"<%=request.getContextPath()%>/common/commonCode_loadCodesTree.action",
				fields:[{
...
isc.MatrixTreeGrid.create({
		...
		getIcon:function(node){//根据节点类型[type dataType]返回不同类型的icon
		  if(node.type==0){ //根节点，表单模型
		      return Matrix.getTreeNodeIcon("folder");
		  }else if(node.type==1){//目录
		  	  return Matrix.getTreeNodeIcon("folder");
		  }else if(node.type==2){//基本
		  	return Matrix.getTreeNodeIcon("code_basic");
		  }else if(node.type==3){//自定义
		  	return Matrix.getTreeNodeIcon("code_define");
		  }
		  //else if(node.type==4){//基本项 不显示基本项
		  //	return Matrix.getTreeNodeIcon("code_define");
		  //}
		}
	});
	...
	<tr id="j_id12" jsId="j_id12">
        ...
        <div id="dataFormCleanButton_div" class="matrixInline matrixInlineIE"
			style="position: relative;; width: 100px;; height: 32px;">
			<script>
			isc.Button.create({
					ID:"MdataFormCleanButton",
					name:"dataFormCleanButton",
					title:"清除",
					displayId:"dataFormCleanButton_div",
					position:"absolute",
					top:0,left:0,width:"100%",height:"100%",
					icon:Matrix.getActionIcon("save"),
					showDisabledIcon:false,
			...
			MdataFormCleanButton.click=function(){
					Matrix.showMask();
					cleanByButton();
