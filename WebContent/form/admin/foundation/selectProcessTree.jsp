...
<script type="text/javascript">
     ...
     function submitByButton(){
	    ...
	    if(nodeDataValue!=null&&nodeDataValue.length>0){
			...
			closeFunction(nodeDataValue);
			parent.layer.close(parent.layer.getFrameIndex(window.name));
	    	
	    }else{
	      isc.say("请选择流程!",{width:150,height:70});
	    }
     ...
     function nodeDoubleClickCustomFun(record){
     	if(record.isFolder == false){
     		var iframewindowid = document.getElementById('iframewindowid').value;
			var closeFunction = eval("parent.on"+iframewindowid+"Close");
			closeFunction(isc.JSON.encode(record));
			parent.layer.close(parent.layer.getFrameIndex(window.name));
     	}else{
     	    isc.say('请选择流程!',{ width:150,height:70});
     	}
...
<script> 
	var MForm0=isc.MatrixForm.create({
				...
				name:"MForm0",
				position:"absolute",
				action:"<%=request.getContextPath()%>/common/processTmpl_getProcessTmplsForTree.action",
				fields:[{
