						...
						$(document).ready(function() {
			              ...
			              //  	"themes": { "theme": "default", "dots": false, "icons": false },  
			    			...
			    			$("#container").on("select_node.jstree",function(e, data){
								...
								if(thetype == 14){
									document.getElementById('InfoIframe').src='<%=path %>/form/formInfo_loadFormMainPage.action?nodeUuid='+node.id+'&type=1&parentNodeId='+parentId+'&processType=${processType}';
								}
								if(thetype == 15){
								...
								if(thetype == 17){
									document.getElementById('InfoIframe').src='<%=path %>/form/admin/process/mainProcess.jsp?processId='+node.original.mid+'&processTmplId='+node.id+'&entityId='+node.id+'&parentNodeId='+parentId+'&type=17&processType=${processType}';
								}
			    			}).jstree();
						});
					</script>
		...
		var type4_8 = {
			...
			"open":{
				...
				"action":function(data){
					...
					var thetype = node.original.type;
					if(thetype == 14){
						$('iframe').attr("src",'<%=path %>/form/formInfo_loadFormMainPage.action?nodeUuid='+node.id+'&type=1&parentNodeId='+parentId+'&processType=${processType}');
					}
					if(thetype == 15){
					...
					if(thetype == 17){
						$('iframe').attr("src",'<%=path %>/form/admin/process/mainProcess.jsp?processId='+node.original.mid+'&processTmplId='+node.id+'&entityId='+node.id+'&parentNodeId='+parentId+'&type=17&processType=${processType}');
					}
