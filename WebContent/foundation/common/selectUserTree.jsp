...
<script> 
	var MForm0=isc.MatrixForm.create({
				...
				name:"MForm0",
				position:"absolute",
				action:"<%=request.getContextPath()%>/common/userSelected_getUsersForTree.action",
				fields:[{
...
isc.MatrixTreeGrid.create({
		...
		getIcon:function(node){//根据节点类型[type dataType]返回不同类型的icon
		  ...
		  }else if(node.isFolder ==true){//机构
		  	//  return Matrix.getTreeNodeIcon("folder");
		  	  return isc.Page.getImgURL("http://localhost:9090/demo/resource/isomorphic/skins/Enterprise/images/matrix/tree/root.png");
		  }else if(node.isFolder ==false){//用户
		  	   //return Matrix.getTreeNodeIcon("user");
		  	   return isc.Page.getImgURL("http://localhost:9090/demo/resource/isomorphic/skins/Enterprise/images/matrix/tree/user.png");
		  }
		}
	});
