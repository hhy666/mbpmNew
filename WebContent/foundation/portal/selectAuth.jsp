...
<script type="text/javascript">
     ...
     function submitByButton(){
	     ...
	     if(nodeDataValue!=null&&nodeDataValue.length>0){
	     	//isc.JSON.decode(),把json字符串解码成json对象，就可以用对象.属性，来得到属性的值了
	   	 	var jsonData=isc.JSON.decode(nodeDataValue);
	    	document.getElementById('Form0').action = 
	    		"<%=request.getContextPath()%>/tenant/tenantAction_saveAdmin.action?userId="+jsonData.id+"&mid="+getMid();
	    	 Matrix.send('Form0',null,function(data){
	    		var success = isc.JSON.decode(data.data);
