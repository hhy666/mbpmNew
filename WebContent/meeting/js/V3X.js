...
//		}
	...
	if(getA8Top().isCtpTop || (typeof(getA8Top().$) != "undefined" && getA8Top().$.dialog)){
	...
	//解决BUG：OA-69691 新建讨论的时候，先滚动滚动条 然后点击加入附件 问题，附件添加框看不见头部，也不能滚到顶端
	//与雷工沟通，暂先去掉resizeFckeditor()
	//resizeFckeditor();  
}
...
function editAttachments(atts,reference,subReference,category){
	...
	var result = v3x.openWindow({
		url		: getA8Top().v3x.baseURL + "/genericController.do?ViewPage=common/fileUpload/attEdit&category="+category+"&reference="+reference
		+"&subReference="+subReference+"&_isModalDialog=true",
		width	: 550,
		height	: 430,
		resizable	: "yes"
	});
	...
	if (this.buttons != null && this.buttons.length > 0) {
		...
		for (var i = 0; i < this.buttons.length; i++) {
            ...
            if (jsonTemp.emphasize) {
				botton.className="button-default_emphasize";
            }
			if (jsonTemp.text) {
...
function openCtpWindow(args) {
	...
	this.workSpaceheight =  parseInt(screen.height)-80 - this.workSpaceTop;
	this.settings = {
		dialog_type : "open",
		resizable : "yes",
		scrollbars : "yes"
	...
	if (html) {
		...
		win.focus();
		win.ctpWinBackParam = args['ctpWinBackParam'];
