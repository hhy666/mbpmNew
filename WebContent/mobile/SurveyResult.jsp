...
<link href="<%=path %>/office/survery/survey.css" rel="stylesheet" type="text/css"/>
<style type="text/css">
...
<script>
	function backToVote(invesInfoId){
	 ...
	 {	
		...
		window.location.href=webContextPath+"/mobile/questions-list.jsp?invesInfoId="+invesInfoId+"&infoRes="+infoRes+"&readOnly=readOnly";
	  }else
	  {
...
<script>
 var Mform0=isc.MatrixForm.create({
	...
	<table width="100%" height="100%" cellspacing="0" cellpadding="0" border="0" class="main-bg" style="height: calc(100% - 43px);">
		...
		<tr>
			<td class="border-top" style="width:10%;height:100%;">
				<!-- 该td是内容区域的td -->	
													...
													<table border="0" width="100%" cellpadding="0" cellspacing="0" align="center" class="">
														<tbody>
															...
															<tr bgcolor="#ffffff" align="left" class="bbs-title-bar , bbs-td-center">
																...
																<!-- 调查题目div -->
																	<div style="width: 100%;overflow:hidden; text-overflow:ellipsis; cursor: hand;height:25px;line-height: 25px;vertical-align: middle;padding-left: 5px;" title="<%=name %>">
															...
															<%
																...
																for(int x = 0;x<subjectOptions.size();x++){
																	...
																	if(optionName!=null && optionName.trim().length()>15){
																		optionName = optionName.trim().substring(0,15)+"……";
																	}
															...
															<tr bgcolor="#ffffff">
																...
																<td>
	                                                   				<img id="sss" src="<%=path %>/resource/images/<%=mod %>.gif" width="<%=width %>" height="15px">
																</td>
																<td>
																	<%=voteNum %>
																</td>
																				...
																				<tr class="sort">
																					...
																					<%
																						if(isAdmin){//是管理员  添加删除操作
																							%>
																								<td width="10%" type="Date" align="left"> 操作</td>
																							<%
																						}else{
																							%>
																								<td width="10%" type="Date" align="left"></td>
																							<%
																	...
																	for(DataObject comment:comments){
																		...
																		<!-- 循环tr -->
																				<tr class="sort" erow="" id="<%=commmentId %>">
																					<td title="<%=content %>" class="sort " width="50%"><%=content %></td>
																					<td align="left" class="sort " width="25%"><%=joinDateStr %></td>
																					<%
																						if("1".equals(voteStyle)) {		//实名															
																					%>
																					 	<td  class="sort " width="15%"><%=userName %></td>
																					<%  
																						} 
																					%>
																					...
																					<%
																						if(isAdmin){//是管理员  添加删除操作
																							%>
																								<td width="10%" type="Date" align="left"><a style="text-decoration:none" width="100%" href=javascript:deleteThisComment('<%=commmentId %>')>删除</a></td>
																							<%
																						}else{
																							%>
																							<td width="10%" type="Date" align="left"></td>
														...
														}else if("2".equals(selectWay)){   //问答式
															%>		
															<table border="0" width="100%" cellpadding="0" cellspacing="0" align="center" class="border-top border-left border-right">
																<tbody>
																	<tr bgcolor="#ffffff" align="left" class="bbs-title-bar , bbs-td-center">
																		<th >
																			<div style="width: 100%;overflow:hidden; text-overflow:ellipsis; cursor: hand;height:25px;line-height: 25px;vertical-align: middle;padding-left: 5px;" title="业务知识的学习">
																				<nobr><%=i+1 %>、<%=name %>（<%=selectWayName %>）
																				</nobr>
																			</div>
																		</th>
																...
																%>
																	<!-- 循环tr -->
																						...
																						<%if("1".equals(voteStyle)) {%>
																							...
																							<td align="left" class="sort" type="Date" width="30%"><%=date %></td>
																							<%
																						}else{
																						%>
																						<td title="<%=content %>" class="sort " width="50%"><%=content %></td>
																							<td align="left" class="sort " width="30%"><%=date %></td>
																							<%
																							if(isAdmin){//是管理员  添加删除操作
																								%>
		...
		<tr>
			<td style="height:25px;width:100%" colspan="2">
			 ...
			 %>
										<td class="bg-advance-bottom" style="padding-left: 10px;background:#e7eaec;">
参加调查人数 <font color="red"><%=allUserNum %> </font>，目前共有 <font color="red"><%=votedUserNum %></font> 人投票，<font color="red"><%=notVoteNum %></font> 人未投票
                 						</td> 
			
					
	
										<td class="bg-advance-bottom" align="right" height="42" style="background:#e7eaec;">						
