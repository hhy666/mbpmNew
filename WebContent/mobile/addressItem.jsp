   ...
   matrix.SERVER_PATH='<%=request.getContextPath() %>';
		...
		<div class="mui-content" style="height:calc(100% - 44px)">
		 <header class="mui-bar mui-bar-nav">
                ...
                document.getElementById('backIcon').addEventListener('tap', function() {
					...
					window.location.href = '<%=request.getContextPath() %>/mobile/address-list.jsp?type=address&parentId='+parentId+'&is_mobile_request=true';
           		});
                </script>
            </header>
	...
	<br>
			<div id="datalist" class="scroll-wrapper" style="top: 42px;overflow:hidden;width:100%;height:100%;background-color:#FFFFFF;" onchange="">
				  ...
				  <iframe id="formContent" style="width: 100%; height: 100%;border:0;" src="<%=request.getContextPath()%>/addressItem.rform?mBizId=<%=uuid%>&mHtml5Flag=true&matrix_view_flag=true"/>  
