...
<div id='matrixMask' name='matrixMask' class='loading'></div>
		...
		<script>
		  ...
		  var parentId = "";
		   ...
		   window.onload = initParam();
			...
			function archiveList(catalogId){
				type = "archive";
				parentId = catalogId;
				clearData();
				var content = "<div class='mui-pull'><div class='mui-pull-loading mui-icon mui-spinner mui-hidden'></div><div class='mui-pull-caption mui-pull-caption-nomore'>正在加载...</div></div>";
				if(document.getElementById("mui-backdrop").children[1])
					document.getElementById("mui-backdrop").children[1].innerHTML=content;
				loadData();
			}
...
//					},
			...
			function getRecordHTML(record){
				...
				if(dataType=="archive"){
					...
					if(record.type == 1){ //具体文档
					    if(record.archiveType == 3){ //关联文档
					...
					}else if(record.type == 2){ //目录
						exeUrl+="<%=request.getContextPath()%>/archive-list.jsp?type=archive&parentId=";
						exeUrl+=record.uuid;
					}
					...
					//var exeUrl = "javascript:archiveList('"+record.mBizId+"');";//æ¥çurl
