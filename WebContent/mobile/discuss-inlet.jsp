...
<div id='matrixMask' name='matrixMask' style="background:#000; filter:alpha(opacity:0); opacity:0;display:none;" class='loading'></div>	        
	...
	<div class="mui-inner-wrap">
		...
		<a id="discusslist" class="mui-control-item" href="#item3mobile">
				所有讨论
			</a>
		<a id="discusscolumn" class="mui-control-item" href="#item3mobile">
				讨论栏目
			</a>
	</div>
	<br>
  ...
  <span class="mui-number">1/2</span>
		...
		<script>
		   ...
		   function initParam(){
	   	      ...
	   	      var discusslist = document.getElementById('discusslist');
	   	      var discusscolumn = document.getElementById('discusscolumn');
	   	      if(type == 'discusslist'){
	   	    	 discusslist.classList.add('mui-active');
	   	      }else if(type == 'discusscolumn'){
	   	    	discusscolumn.classList.add('mui-active');
	   	      }
		   }
		   window.onload = initParam();
			...
			//监听标签切换
			document.getElementById('discusslist').addEventListener('tap',function(){
			   type = "discusslist";
			   var startNumValue = 0; 
			...
			document.getElementById('discusscolumn').addEventListener('tap',function(){
				type = "discusscolumn";
				var startNumValue = 0; 
				document.getElementById("startNum").value = startNumValue;
			...
			function loadData(){
				...
				if(dataType=="discusslist" ){        //所有讨论
					...
					matrix.sendRequest("<%=request.getContextPath()%>/mobile/discuss_queryAllDiscuss.action",data,loadDataCallback);

				}else if(dataType=="discusscolumn"){    //讨论栏目
					matrix.showMask();
					var data = {};
					data["queryStartNum"] = startNum * matrix.DEFAULT_PAGE_SIZE;
					data["queryPageSize"] = matrix.DEFAULT_PAGE_SIZE;
					...
					matrix.sendRequest("<%=request.getContextPath()%>/mobile/discuss_queryCatalogs.action",data,loadDataCallback);
...
//						callback: pulldownRefresh
			...
			function getRecordHTML(record){
				if(dataType=="discusslist"){    //所有讨论
					//å¾åä»»å¡
					var exeUrl = "";//æ§è¡url
					exeUrl+="<%=request.getContextPath()%>/mobile/discuss.jsp?";
					exeUrl+="uuid=";
...
//					row_title.href = "#";
					...
					var imgsrc = "<%=request.getContextPath()%>/resource/mobile/cover_8.png";
					...
					var titleSpan = document.createElement('b');
					//nameDiv.className = "mui-media-body";
					titleSpan.innerHTML = record.title;      //讨论标题
					...
					nameSpan.innerHTML = record.starterDep;     //发布人所在部门名称
					userDiv.appendChild(nameSpan);
					
					var dateSpan = document.createElement('span'); 
					dateSpan.setAttribute("style","float:right;");   //发表时间
					dateSpan.innerHTML = record.createDate;
					...
					//æ ç®
					var exeUrl = "<%=request.getContextPath()%>/mobile/discuss-list.jsp?type=discuss&parentId="+record.mBizId;
					//var exeUrl = "javascript:discussList('"+record.mBizId+"');";//æ¥çurl
					exeUrl = matrix.convertRequestUrl(exeUrl);
					var row = document.createElement('li');
					...
					var imgsrc = "<%=request.getContextPath()%>/resource/mobile/taolun.png";
					img.src = matrix.convertRequestUrl(imgsrc);
					row_title.appendChild(img);
