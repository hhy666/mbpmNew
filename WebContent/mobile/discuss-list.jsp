    ...
    <div class="mui-off-canvas-wrap mui-draggable " id="offCanvasWrapper">
		...
		<div class="mui-content">
		 <header class="mui-bar mui-bar-nav">
                ...
                <h1 id="title" class="mui-title">讨论栏目</h1>
                <script type="text/javascript">
					//返回按钮点击事件
					document.getElementById('backIcon').addEventListener('tap', function() {
						var type = matrix.getParam("type");
						var parentId = matrix.getParam("parentId");
						if(type == "discuss"){    //讨论列表
							window.location.href = '<%=request.getContextPath() %>/mobile/discuss-inlet.jsp?type=discusscolumn';
						}else if(type == "catalog"){   //讨论栏目
		...
		<script>
		  ...
		  var parentId = "";
		   function initParam(){
	   	      ...
	   	      if(type == "discuss"){
				document.getElementById("title").innerText="讨论列表";
	   	      }else if(type == "catalog"){
				document.getElementById("title").innerText="讨论栏目";
	   	      }
		   ...
		   window.onload = initParam();
			...
			document.getElementById('page-next').addEventListener('tap',function(){
			   ...
			   //å¦æå½åå·²ç»æ¯æåä¸è¡,ä»ä¹ä¹ä¸å
			...
			function discussList(catalogId){
				type = "news";
				document.getElementById("title").innerText="调查列表";
				parentId = catalogId;
				clearData();
				loadData();
			}
			...
			function loadData(){
				...
				if(dataType=="catalog"){
					...
					matrix.sendRequest("<%=request.getContextPath()%>/mobile/discuss_queryDiscuss.action",data,loadDataCallback);
