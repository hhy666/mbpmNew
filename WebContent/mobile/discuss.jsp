...
.demo-iframe-holder iframe {  
  ...
  width: 100%;  
	...
	<body>
<%
  String uuid = request.getParameter("uuid");   //讨论主键
   ...
   matrix.SERVER_PATH='<%=request.getContextPath() %>';
		 ...
		 <header class="mui-bar mui-bar-nav">
                ...
                document.getElementById('backIcon').addEventListener('tap', function() {
                	...
                	if(parentId!='' && parentId!='null'){
                		window.location.href = '<%=request.getContextPath() %>/mobile/discuss-list.jsp?type=discuss&parentId='+parentId+'&is_mobile_request=true';
                	}else{
                		window.location.href = '<%=request.getContextPath() %>/mobile/discuss-inlet.jsp?type=discusslist';
                	}
	...
	<br>
			<div id="datalist" class="scroll-wrapper" style="top: 42px;overflow:hidden;width:100%;height:100%;background-color:#FFFFFF;" onchange="">
				  ...
				  <iframe id="formContent" style="width: 100%; height: 100%;border:0;" src="<%=request.getContextPath() %>/mobile/discuss-detailInfo.jsp?uuid=<%=uuid%>&parentId=<%=parentId%>&is_mobile_request=true&isClick=1"/>  
