...
<div id='matrixMask' name='matrixMask' style="background:#000; filter:alpha(opacity:0); opacity:0;display:none;" class='loading'></div>	        
	...
	<div class="mui-inner-wrap">
		...
		<a id="unReaded" class="mui-control-item mui-active" href="#item3mobile">
				未读消息
			</a>
		<a id="readed" class="mui-control-item" href="#item3mobile">
				已读消息
			</a>
	</div>
	<br>
  ...
  <span class="mui-number">1/2</span>
		...
		<script>
		   ...
		   window.onload = initParam();
			...
			//监听标签切换
			document.getElementById('unReaded').addEventListener('tap',function(){
			   type = "unReaded";
			   var startNumValue = 0; 
			...
			document.getElementById('readed').addEventListener('tap',function(){
			   type = "readed";
			   var startNumValue = 0; 
			   document.getElementById("startNum").value = startNumValue;
			...
			function loadData(){
				...
				if(dataType=="unReaded" ){        //未读消息
					...
					matrix.sendRequest("<%=request.getContextPath()%>/mobile/message_queryUnReaded.action",data,loadDataCallback);

				}else if(dataType=="readed"){    //已读消息
					var data = {};
					...
					matrix.sendRequest("<%=request.getContextPath()%>/mobile/message_queryReaded.action",data,loadDataCallback);
...
//						callback: pulldownRefresh
			...
			function getRecordHTML(record){
				...
				if(dataType=="unReaded"){    //未读消息  
					var exeUrl = "";
					exeUrl+="<%=request.getContextPath()%>/mobile/message.jsp?";
					exeUrl+="messageId=";
					exeUrl+=record.messageId;
					...
					var imgsrc = "<%=request.getContextPath()%>/resource/mobile/unReaded.png";
					...
					titleSpan.innerHTML = record.subjectValue;      //消息标题
					...
					nameSpan.innerHTML = record.userName;     //发送人姓名
					userDiv.appendChild(nameSpan);
					
					var dateSpan = document.createElement('span'); 
					dateSpan.setAttribute("style","float:right;");   //发送时间
					dateSpan.innerHTML = record.createdDate;
					...
					var img = document.createElement('img');
					img.className = "mui-media-object mui-pull-left";
					var imgsrc = "<%=request.getContextPath()%>/resource/mobile/readed.png";
					img.src = matrix.convertRequestUrl(imgsrc);
					row_title.appendChild(img);
