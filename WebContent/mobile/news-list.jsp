       ...
       <!-- 侧滑菜单导航容器 -->
		...
		<div class="mui-content">
		 <header class="mui-bar mui-bar-nav">
                ...
                <h1 id="title" class="mui-title">新闻栏目</h1>
                <script type="text/javascript">
					...
					document.getElementById('backIcon').addEventListener('tap', function() {
						...
						if(type == "news"){    //新闻列表
							window.location.href = '<%=request.getContextPath() %>/mobile/news-inlet.jsp?type=newscolumn';
						}else if(type == "catalog"){   //新闻栏目
		...
		<script>
		  ...
		  var parentId = "";
		   function initParam(){
	   	      ...
	   	      if(type == "news"){
				document.getElementById("title").innerText="新闻列表";
	   	      }else if(type == "catalog"){
				document.getElementById("title").innerText="新闻栏目";
	   	      }
		   ...
		   window.onload = initParam();
			...
			function loadData(){
				...
				if(dataType=="catalog"){
					...
					matrix.sendRequest("<%=request.getContextPath()%>/mobile/news_queryNews.action",data,loadDataCallback);
