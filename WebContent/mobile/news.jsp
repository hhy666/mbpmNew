		 ...
		 <header class="mui-bar mui-bar-nav" id="headTitle">
                ...
                document.getElementById('backIcon').addEventListener('tap', function() {
                	...
                	if(type == 'archive'){   //文档列表查看
                		...
                		if(parentId!='' && parentId!='null'){
                    		window.location.href = '<%=request.getContextPath() %>/mobile/news-list.jsp?type=news&parentId='+parentId+'&is_mobile_request=true';
                    	}else{
                    		window.location.href = '<%=request.getContextPath() %>/mobile/news-inlet.jsp?type=newslist';
                    	}
                	}
			...
			<div id="datalist" class="scroll-wrapper" style="top: 42px;overflow:hidden;width:100%;height:100%;background-color:#FFFFFF;" onchange="">
				  ...
				  <iframe id="formContent" style="width: 100%; height: 100%;border:0;" src="<%=request.getContextPath() %>/CustomerVelocityServlet?mBizId=<%=uuid%>&type=2&isPreview=true&status=1"/>  
