...
<div id='matrixMask' name='matrixMask' style="background:#000; filter:alpha(opacity:0); opacity:0;display:none;" class='loading'></div>	        
	...
	<div class="mui-inner-wrap">
		...
		<a id="noticelist" class="mui-control-item" href="#item3mobile">
				所有公告
			</a>
		<a id="noticecolumn" class="mui-control-item" href="#item3mobile">
				公告栏目
			</a>
	</div>
	<br>
  ...
  <span class="mui-number">1/2</span>
		...
		<script>
		   ...
		   function initParam(){
	   	      ...
	   	      var noticecolumn = document.getElementById('noticecolumn');
	   	      if(type == 'noticelist'){
	   	    	 noticelist.classList.add('mui-active');
	   	      }else if(type == 'noticecolumn'){
	   	    	noticecolumn.classList.add('mui-active');
	   	      }
		   }
		   window.onload = initParam();
			...
			//监听标签切换
			document.getElementById('noticelist').addEventListener('tap',function(){
			   type = "noticelist";
			   var startNumValue = 0; 
			...
			document.getElementById('noticecolumn').addEventListener('tap',function(){
				type = "noticecolumn";
				var startNumValue = 0; 
				document.getElementById("startNum").value = startNumValue;
			...
			function loadData(){
				...
				if(dataType=="noticelist" ){        //所有公告
					...
					matrix.sendRequest("<%=request.getContextPath()%>/mobile/notice_queryAllNotices.action",data,loadDataCallback);

				}else if(dataType=="noticecolumn"){    //公告栏目
					matrix.showMask();
					var data = {};
					data["queryStartNum"] = startNum * matrix.DEFAULT_PAGE_SIZE;
					data["queryPageSize"] = matrix.DEFAULT_PAGE_SIZE;
					...
					matrix.sendRequest("<%=request.getContextPath()%>/mobile/notice_queryCatalogs.action",data,loadDataCallback);
...
//						callback: pulldownRefresh
			...
			function getRecordHTML(record){
				if(dataType=="noticelist"){    //所有公告
					//å¬å
					var exeUrl = "";//æ§è¡url
					exeUrl+="<%=request.getContextPath()%>/mobile/note.jsp?";
					exeUrl+="mBizId=";
...
//					row_title.href = "#";
					...
					var imgsrc = "<%=request.getContextPath()%>/resource/mobile/cover_6.png";
					...
					//æ ç®
					var exeUrl = "<%=request.getContextPath()%>/mobile/notice-list.jsp?type=notice&parentId="+record.mBizId;
					exeUrl = matrix.convertRequestUrl(exeUrl);
					//var exeUrl = "javascript:noticeList('"+record.mBizId+"');";//æ¥çurl
					...
					var imgsrc = "<%=request.getContextPath()%>/resource/mobile/notice.png";
					img.src = matrix.convertRequestUrl(imgsrc);
					row_title.appendChild(img);
