...
.demo-iframe-holder iframe {  
  ...
  width: 100%;  
	...
	<body>
<%
  String mBizId = request.getParameter("mBizId");   //查询信息主键
       ...
       <!-- 侧滑菜单导航容器 -->
		...
		<div class="mui-content" style="height:calc(100% - 44px)">
		 <header class="mui-bar mui-bar-nav">
                ...
                <h1 class="mui-title">查询信息详情</h1>
                 <script type="text/javascript">
                document.getElementById('backIcon').addEventListener('tap', function() {
					window.location.href = '<%=request.getContextPath() %>/mobile/query_loadDataQuery.action';
           		});
                </script>
            </header>
			...
			<div id="datalist" class="scroll-wrapper" style="top: 42px;overflow:hidden;width:100%;height:100%;background-color:#FFFFFF;" onchange="">
				  <iframe id="formContent" style="width: 100%; height: 100%;border:0;" src="<%=request.getContextPath() %>/matrix.rform?configUUID=<%=mBizId %>&formId=<%=formId %>&mIsQueryMode=true&mHtml5Flag=true"/>  
		   </div>
		</div>
