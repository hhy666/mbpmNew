<%@page import="com.matrix.form.admin.custom.queryset.model.QuerySetInfo"%>
...
<script type="text/javascript">
	...
	function openUtilizationList(mBizId,formId,mobileform){
		//如果是自定义移动表单
		if(mobileform == 2){
			formId = formId + "_mobile";
		}
		window.location.href = '<%=request.getContextPath() %>/mobile/query-info.jsp?mBizId='+mBizId+'&formId='+formId;
	}
...
<div class='' id='offCanvasWrapper'>
				...
				<tbody>
					<tr>
						<td width="100%" valign="top" class="border_b padding_l_10" style="border:0px;">
							<form id="searchForm" method="post" action="<%=request.getContextPath()%>/mobile/query_loadDataQuery.action">
		...
		<%
			try {
				...
				List<QuerySetInfo> dataList = (List<QuerySetInfo>) request.getAttribute("dataList");   //应用信息列表
		%>
		
		<%
			for (DataObject dataObject : leafList) {
					List<QuerySetInfo> list = new ArrayList<QuerySetInfo>();   //存放应用信息
					HashMap<QuerySetInfo, Integer> keyUtil = new HashMap<QuerySetInfo, Integer>();
					for(int i=0; i<templateList.size(); i++){
						...
						if(parentId.equals(dataObject.getString("uuid"))){
							for (QuerySetInfo querySetInfo : dataList) {
								String catalogId = querySetInfo.getCatalogId();
								if(catalogId!=null && catalogId.toString().trim().length()>0){
									if(catalogId.equals(mBizId)){
										list.add(querySetInfo);
										Integer mobileform = (Integer) temp[2];   //移动表单类型   1原表单  2自定义  3不支持
										keyUtil.put(querySetInfo, mobileform);
									}
								}
							}	
						}
 	...
 	if (i * 2 < list.size()) {
 %> <span><img src="<%=request.getContextPath()%>/resource/mobile/cover_11.png"/></span><a
													class="title-already-visited"
													style="text-decoration: none; color: #000000"
													href=javascript:openUtilizationList('<%=list.get(i * 2).getUuid()%>','<%=list.get(i * 2).getFormId()%>','<%=keyUtil.get(list.get(i * 2)) %>')>

														<%=list.get(i * 2).getQueryName()%></a> <%
 	}
 %></td>
												<td width="25%" style="color: rgb(51, 51, 51);"
 ...
 %> <span><img src="<%=request.getContextPath()%>/resource/mobile/cover_11.png"/></span><a
													class="title-already-visited"
													style="text-decoration: none; color: #000000"
													href=javascript:openUtilizationList('<%=list.get(i * 2 + 1).getUuid()%>','<%=list.get(i * 2).getFormId()%>','<%=keyUtil.get(list.get(i * 2)) %>')>

														<%=list.get(i * 2 + 1).getQueryName()%></a> <%
 	}
 %></td>											
