...
<form id="form0" name="form0" eventProxy="Mform0" method="post" onsubmit="return checkContent()"
	action="<%=request.getContextPath()%>/investigation/investigation_saveInvestigationQuestion.action?parentId=<%=request.getParameter("parentId")%>"
	...
	<table width="100%" height="100%" cellspacing="0" cellpadding="0" border="0" class="main-bg">
		...
		<tr>
			<td class="border-top" style="vertical-align: top;">
				<div style="overflow: auto; height: calc(100% - 60px); width: 100%;" class="scrollList" id="scrollListDiv">
					<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0" class="">
						<tr>							
							<td width="100%" valign="top" class=" top-padding" style="padding: 10px;">
...
%>
	...
	<script>
		...
		<tr>
		  <td colspan="2" class="cmdLayout">
		    <table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" style="margin-right:0px;position: absolute;">
			 <tbody>
			  <tr>
			 <%
			 ...
			 if(publicDepId!=null && publicDepId.trim().length()>0){
						 ...
						 %>
			 <td style="text-align:right;padding-right: 10px;background:#e7eaec;border: 1px solid #e7eaec">
	...
	<tr>
	<td align="right" height="42" style="padding-right: 10px;background:#e7eaec;">
	<%
		//=========
	...
	%>	
		...
		<%
		String infoRes = request.getParameter("infoRes");
		//if(infoRes !=null && "1".equals(infoRes)){  //infoRes==1说明是从调查管理入口进入的
		%>
...
<script>
    ...
    location.replace(webContextPath+"/mobile/SurveyResult.jsp?infoRes="+infoRes+"&invesInfoId="+invesInfoId);
}
