...
<head>
		...
		//双击选择角色
		function doubleClick2SelectRole(){
			var select  = MTree001.getSelection();
			if(select!=null && select.length>0){
				var gridDataList = parent.parent.MDataGrid005.getData();
				var newData = {};
				newData.userId = select[0].entityId;
				newData.userName = select[0].text;
...
<script>
 var Mform0=isc.MatrixForm.create({
 				...
 				name:"Mform0",
 				position:"absolute",
 				action:"<%=request.getContextPath()%>/editor/editor_selectRole.action",
 				fields:[{
...
<table id="table001" class="tableLayout"
	...
	<tr id="tr001">
		<td id="td001" class="tdLayout" style="height: 100%; width: 100%;">
