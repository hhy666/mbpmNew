...
<div id='matrixMask' name='matrixMask' style="background:#000; filter:alpha(opacity:0); opacity:0;display:none;" class='loading'></div>	        
	...
	<div class="mui-inner-wrap">
		...
		<a id="surverylist" class="mui-control-item" href="#item3mobile">
				所有调查
			</a>
		<a id="surverycolumn" class="mui-control-item" href="#item3mobile">
				调查栏目
			</a>
	</div>
	<br>
  ...
  <span class="mui-number">1/2</span>
		...
		<script>
		   ...
		   function initParam(){
	   	      ...
	   	      var surverylist = document.getElementById('surverylist');
	   	      var surverycolumn = document.getElementById('surverycolumn');
	   	      if(type == 'surverylist'){
	   	    	 surverylist.classList.add('mui-active');
	   	      }else if(type == 'surverycolumn'){
	   	    	surverycolumn.classList.add('mui-active');
	   	      }
		   }
		   window.onload = initParam();
			...
			//监听标签切换
			document.getElementById('surverylist').addEventListener('tap',function(){
			   type = "surverylist";
			   var startNumValue = 0; 
			...
			document.getElementById('surverycolumn').addEventListener('tap',function(){
				type = "surverycolumn";
				var startNumValue = 0; 
				document.getElementById("startNum").value = startNumValue;
			...
			function loadData(){
				...
				if(dataType=="surverylist" ){        //所有调查
					...
					matrix.sendRequest("<%=request.getContextPath()%>/mobile/survery_queryAllSurvery.action",data,loadDataCallback);

				}else if(dataType=="surverycolumn"){    //调查栏目
					matrix.showMask();
					var data = {};
					data["queryStartNum"] = startNum * matrix.DEFAULT_PAGE_SIZE;
					data["queryPageSize"] = matrix.DEFAULT_PAGE_SIZE;
					...
					matrix.sendRequest("<%=request.getContextPath()%>/mobile/survery_queryCatalogs.action",data,loadDataCallback);
...
//						callback: pulldownRefresh
			...
			function getRecordHTML(record){
				if(dataType=="surverylist"){    //所有调查
					//å¾åä»»å¡
					var exeUrl = "";//æ§è¡url
					exeUrl+="<%=request.getContextPath()%>/mobile/survery.jsp?";
					exeUrl+="invesInfoId=";
...
//					row_title.href = "#";
					...
					var imgsrc = "<%=request.getContextPath()%>/resource/mobile/cover_7.png";
					...
					titleSpan.innerHTML = record.title;      //调查标题
					...
					//æ ç®
					var exeUrl = "<%=request.getContextPath()%>/mobile/survery-list.jsp?type=survery&parentId="+record.mBizId;
					//var exeUrl = "javascript:surveryList('"+record.mBizId+"');";//æ¥çurl
					exeUrl = matrix.convertRequestUrl(exeUrl);
					var row = document.createElement('li');
					...
					var imgsrc = "<%=request.getContextPath()%>/resource/mobile/survery.png";
					img.src = matrix.convertRequestUrl(imgsrc);
					row_title.appendChild(img);
