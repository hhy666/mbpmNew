...
.demo-iframe-holder iframe {  
  ...
  width: 100%;  
	...
	<body>
<%
  String invesInfoId = request.getParameter("invesInfoId");  //调查主键
   ...
   matrix.SERVER_PATH='<%=request.getContextPath() %>';
    ...
    <div class="scroll-wrapper mui-off-canvas-wrap mui-draggable " id="offCanvasWrapper">
		...
		<div class="mui-content" style="height:calc(100% - 44px)">
		 <header class="mui-bar mui-bar-nav">
                ...
                <h1 class="mui-title">调查详情</h1>
                 <script type="text/javascript">
                document.getElementById('backIcon').addEventListener('tap', function() {
                	var parentId = "<%=parentId%>";
                	if(parentId!='' && parentId!='null'){
                		window.location.href = '<%=request.getContextPath() %>/mobile/survery-list.jsp?type=survery&parentId='+parentId+'&is_mobile_request=true';
                	}else{
                		window.location.href = '<%=request.getContextPath() %>/mobile/survery-inlet.jsp?type=surverylist';
                	}
	...
	<br>
			<div id="datalist" class="scroll-wrapper" style="top: 42px;overflow:hidden;width:100%;height:100%;background-color:#FFFFFF;" onchange="">
				  ...
				  <iframe id="formContent" style="width: 100%; height: 100%;border:0;" src="<%=request.getContextPath() %>/mobile/questions-list.jsp?invesInfoId=<%=invesInfoId%>&parentId=<%=parentId%>"/>  
