...
<script type="text/javascript">
	...
	function openUtilizationList(mBizId,formId,mobileform){
		...
		window.location.href = '<%=request.getContextPath() %>/mobile/utilization-info.jsp?mBizId='+mBizId+'&formId='+formId+'&is_mobile_request=true';
	}
...
<div class='' id='offCanvasWrapper'>
				...
				<tbody>
					<tr>
						<td width="100%" valign="top" class="border_b padding_l_10" style="border:0px;">
							<form id="searchForm" method="post" action="<%=request.getContextPath()%>/mobile/utilization_loadDataUtilization.action">
		...
		<%
			try {
				...
				List<Utilization> dataList = (List<Utilization>) request.getAttribute("dataList");   //应用信息列表
		%>
		
		<%
			for (DataObject dataObject : leafList) {
					List<Utilization> list = new ArrayList<Utilization>();   //存放应用信息
					HashMap<Utilization, Integer> keyUtil = new HashMap<Utilization, Integer>();
					for(int i=0; i<templateList.size(); i++){
						...
						if(parentId.equals(dataObject.getString("uuid"))){
							for (Utilization utilization : dataList) {
								String catalogId = utilization.getCatalogId();
								if(catalogId!=null && catalogId.toString().trim().length()>0){
									if(catalogId.equals(mBizId)){									
										...
										keyUtil.put(utilization, mobileform);
									}
								}
							}	
						}
 	...
 	if (i * 2 < list.size()) {
 %> <span><img src="<%=request.getContextPath()%>/resource/mobile/cover_10.png"/></span><a
 ...
 %> <span><img src="<%=request.getContextPath()%>/resource/mobile/cover_10.png"/></span><a
													class="title-already-visited"
													style="text-decoration: none; color: #000000"
													href=javascript:openUtilizationList('<%=list.get(i * 2 + 1).getUuid()%>','<%=list.get(i * 2 + 1).getFormId()%>','<%=keyUtil.get(list.get(i * 2)) %>')>
