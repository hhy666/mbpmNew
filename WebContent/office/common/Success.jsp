...
<%
   ...
   if(piid != null){  //流程中
%>
	parent.window.location="<%=request.getContextPath()%>/mobile/flow-list.jsp?type=readyTask";
<%
   }else{  //发起流程
%>
	parent.window.location="<%=request.getContextPath()%>/mobile/app-list.jsp?type=newApp&parentId=flowRoot";
<%
   }
%>
}
  ...
  <input type="hidden" id="result" name="result" value="<%=request.getAttribute("result") %>"/>
    ...
    <% 
    if(com.matrix.form.util.CommonUtil.isMobile()){
    	%>
  ...
  <div class="hint clr" style="background:#f3f3f3;text-align:center;">
    ...
    %>
     ...
     <div class="hint clr">
        ...
        <% String msgInfo = (String)request.getAttribute("errorInfo");
if(msgInfo != null && msgInfo.trim().length()>0){
%>
            <h1>发送失败</h1>
			<br><h2>提示信息：</h2><%=msgInfo%> 
            <h1><br>请与管理员联系！<a href="javascript:window.close()" class="color_success">关闭</a></h1>
<%}else{%>
            <h1>发送失败<br>请<a href="javascript:window.close()" class="color_success">关闭</a>重新操作！</h1>
<%}%>
        </div>
    </div>
  </div>
  ...
  <%} else if("error1".equals(result)){ %>
   <div class="hint clr">
        <div class="error_icon f_l"><img src="<%=basePath%>image/error_512.png"></div>
        <div class="hint_cont hint_cont_error f_l">
            <h1>流程版本不存在或者未发布<br>请<a href="javascript:window.close()" class="color_success">关闭</a>重新操作！</h1>
        </div>
  </div>
  <%}%>
