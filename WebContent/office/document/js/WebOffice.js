function WebOffice2015() {
    ...
    */
    ...
    this.WebOpen = function (mBoolean) {
        ...
        httpclient.Clear();
         this.WebSetMsgByName("INITTEMPLATEID",this.InitTemplateId);
         this.WebSetMsgByName("TEMPLATEOPERATION",this.TemplateOperation);//是否是正文模板或者套红模板的操作
         this.WebSetMsgByName("WORDTYPE",this.WordType);//正文模板  套红模板
         this.WebSetMsgByName("SAVETYPE",this.SaveType);
         this.WebSetMsgByName("MFLOWBIZID",this.FlowBizId);
         this.WebSetMsgByName("ISINITLOAD",this.IsInitLoad);
         this.WebSetMsgByName("ENTITY",this.Entity);//加载实体   (lpz)
    	 this.WebSetMsgByName("WORDTEMP",this.WordTemp);//加载初始模板编码
    	 this.WebSetMsgByName("VBATEMP",this.RedTempId);//加载初始模板编码
    	 this.WebSetMsgByName("TYPE",this.Type);//判断是否是发文类型
    	 ...
    	 this.WebSetMsgByName("HISTORY",this.History);
    	 this.WebSetMsgByName("TEMPORARY","");//套红阶段，取套红模版时，临时套红模版编码套
    ...
    this.WebSave = function () {
        ...
        httpclient.Clear();               
      //套红参数
         this.WebSetMsgByName("REDTEMPLATEID",this.RedTemplateId);
         this.WebSetMsgByName("VBA",this.VBA);
        ...
        //正文模板
         ...
         this.WebSetMsgByName("TEMPLATEOPERATION",this.TemplateOperation);//是否是正文模板或者套红模板的操作
          this.WebSetMsgByName("WORDTYPE",this.WordType);//正文模板  套红模板
         this.WebSetMsgByName("SAVETYPE",this.SaveType);
         this.WebSetMsgByName("UUID",this.Uuid);
         
         this.WebSetMsgByName("MFLOWBIZID",this.FlowBizId);
         this.WebSetMsgByName("ISINITLOAD",this.IsInitLoad);
         this.WebSetMsgByName("PIID",this.Piid);
         this.WebSetMsgByName("TYPE",this.Type);
         this.WebSetMsgByName("SUBTYPE",this.SubType);
         this.WebSetMsgByName("VBATEMP",this.VbaTemp);
    	 this.WebSetMsgByName("WORDTEMP",this.WordTemp);
    	 this.WebSetMsgByName("ENTITY",this.Entity);              
    	 ...
    	 //this.WebSetMsgByName("CONTENT",this.Content);
    	 this.WebSetMsgByName("FILENAME",this.FileName);//加载FileName
    	 debugger;
    ...
    this.getFilePath = function () {
        ...
        var filePath = "D:/upfile/doc_process/"; //设置临时路径
  ...
  //光标处添加书签
   ...
   //删除书签
    ...
    //根据文档名称插入文档
    this.WebInsertFile = function (filename) {
    	if(filename == "" || filename == null){
    		filename = this.FileName;
    	}
        var httpclient = this.obj.Http; //设置http对象
        this.WebSetMsgByName("FILENAME", filename); //加载FileName
        this.WebSetMsgByName("OPTION", "LOADFILE");     //发送请求LOADFILE
        httpclient.AddForm("FormData", this.WebSendMessage()); //这里是自定义json 传输格式。
    ...
    //套红阶段，下载套红文档
    this.DownloadToFile = function (FullFilePath) {
        var httpclient = this.obj.Http;
   	 ...
   	 this.WebSetMsgByName("RECORDID",this.RecordID);//加载RecordID
   	 this.WebSetMsgByName("OPTION","LOADFILE");     //发送请求LOADFILE
   	this.WebSetMsgByName("TEMPORARY",this.Temporary);     //临时套红模版编码
        
        httpclient.AddForm("FormData", this.WebSendMessage()); //这里是自定义json 传输格式。
        this.WebClearMessage();      
        
        httpclient.ShowProgressUI = false;//隐藏进度条
        if (httpclient.Open(this.HttpMethod.Post, this.WebUrl, false)) {//指定下载模板的名称
            if (httpclient.Send()) {
                if (httpclient.Status == 200) {
                    httpclient.ResponseSaveToFile(FullFilePath);
                    httpclient.Clear();
                    return true;
                }
            }
