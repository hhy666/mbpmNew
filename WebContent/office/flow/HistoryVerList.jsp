...
parent.document.getElementById('hisFrame').style.display="none";
	...
	function look(uuid,plateId,isInitLoad,type) {
//	Matrix.setFormItemValue("uuid",uuid);
parent.look(uuid,plateId,isInitLoad,type);
	}
</script>
	<% 
  ...
  String plateId = MFormContext.getParameter("plateId");//模板编码
  String isInitLoad = MFormContext.getParameter("isInitLoad");//是否初始加载
  String type = MFormContext.getParameter("type");//类型
  List<DataObject> list = dataService.queryList("from office.document.common.ProcessDoc where procInsId='"+mFlowBizId+"' and hisVerFlag=1 order by lastModifyDate asc",null);
...
<div class="stadic_layout_head stadic_head_height" style="background-color:#dadada">
   ...
   <span class="ico16 close_16 cursor-hand" onclick="hideHisList();"></span>
  </td></tr>
  </table>
</div>
 ...
 <table id="hisTable" width="100%"  border="2"  bordercolor="grey" style="border-collapse: collapse;" class="bg_color_white font_size12">
  ...
  <td class="align_left w30b padding_l_5">流程节点名称</td>
  ...
  String uuid = hisObj.getString("mBizId");//唯一编码
  String procNodeId = hisObj.getString("procNodeId");//流程节点名称
  String modifierId = hisObj.getString("modifierId");//修改人编码
  Date lastModifyDate = hisObj.getDate("lastModifyDate");//最后修改时间
  String modifier = (String)dataService.queryValue("select userName from foundation.org.User where userId='"+modifierId+"'",null);//修改人
  SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm");
  String dateStr = sdf.format(lastModifyDate);
  %>

   <tr class="align_center">
  <td class="align_left" style="line-height: 20px;"><%=procNodeId %></td>
  <td><%=modifier %></td>
  <td><%=dateStr %></td>
  <td><span>[<a href="javascript:look('<%=uuid%>','<%=plateId %>','<%=isInitLoad %>','1')">查看</a>]</span></td>
