...
<html>
	<head>
        ...
        var a =startTime.replace(/[年月]/g,"/").replace(/日/g,"");
        startTime = new Date(a).format("yyyy-MM-dd HH:mm:ss");
        var endTime=Matrix.getFormItemValue("endTime").toLocaleDateString();
        var b =endTime.replace(/[年月]/g,"/").replace(/日/g,"")
        endTime = new Date(b).format("yyyy-MM-dd HH:mm:ss");
		...
		var src = "<%=request.getContextPath()%>/office/performance/tabflow.jsp?flowType="+flowType+"&type="+type+"&statisType="+statisType+"&startTime="+startTime+"&endTime="+endTime+"&templetId="+templetId+"&userId="+userId+"&depId="+depId;
...
    }
	...
	isc.VLayout.create( {
		...
		members : [ isc.MatrixHTMLFlow.create( {
			...
			contentsType : "page",
			contentsURL : "<%=request.getContextPath()%>/office/performance/tabflow.jsp?flowType=&status=&startTime=&endTime="
	}) ],
		canSelectText : true
	});
</script>
						...
						<tr id="tr002">
							...
							<td id="td007" class="tdLayout" rowspan="1" style="width: 30%;">
								<div id="startTime_div" eventProxy="Mform0" class="matrixInline"
									style=""></div>
								<script>
	//var MstartTime_value =new Date(new Date().setMonth((new Date().getMonth()-1)));//提前一个月
	var MstartTime_value =new Date();//
