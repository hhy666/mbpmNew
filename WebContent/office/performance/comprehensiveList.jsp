...
<html>
	<head>
		...
		function showPenetration(record){
			if(record!=null){
			Mpenetration.initSrc="<%=request.getContextPath()%>/performance/pen_comprehensive.action?flowType=${param.flowType}&startTime=${param.startTime}&endTime=${param.endTime}&type=${param.type}&templetId="+record.templateId;
			Mpenetration.src="<%=request.getContextPath()%>/performance/pen_comprehensive.action?flowType=${param.flowType}&startTime=${param.startTime}&endTime=${param.endTime}&type=${param.type}&templetId="+record.templateId;
			Matrix.showWindow("penetration");
			}
		}
		</script>
	</head>
	...
	var Mform0 = isc.MatrixForm.create( {
		...
		name : "Mform0",
		position : "absolute",
		action : "<%=request.getContextPath()%>/performance/per_comprehensive.action?flowType=${param.flowType}&startTime=${param.startTime}&endTime=${param.endTime}&type=${param.type}&templetId=${param.templetId}",
		canSelectText : true,
		fields : [ {
		...
		<div
			...
			<form id="form0" name="form0" eventProxy="Mform0" method="post"
				action="<%=request.getContextPath()%>/performance/per_comprehensive.action?flowType=${param.flowType}&startTime=${param.startTime}&endTime=${param.endTime}&type=${param.type}&templetId=${param.templetId}"
				style="margin: 0px; position: relative; overflow: hidden; width: 100%; height: 100%;overflow:auto;"
	...
	<tr style="width: 100%;height:35px;">
		...
		<td class="toolStrip" 
			...
			<script>
				function getParamsForpenetration(){
				var params='&';
				var value;
				return params;
				}
				...
				<script>Mpenetration.hide();</script>
			</form>
		</div>
		<script>
