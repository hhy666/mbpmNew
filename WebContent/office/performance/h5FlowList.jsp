...
<script type="text/javascript">
	function showPenetration(record){
		parent.parent.layer.open({
			...
			title:['穿透查询'],
			area:['85%','85%'],
			content:'<%=path%>/performance/pen_pencountFlow.action?flowType=${param.flowType}&templetId=${param.templetId}&startTime=${param.startTime}&endTime=${param.endTime}&piid='+record.piid+'&depId='+record.depId
		});
	}
</script>
<title>流程统计数据表格</title>
</head>
   			...
   			<script>
    			$(document).ready(function(){
					$("#DataGrid001_table").bootstrapTable({
						...
						url:'<%=path%>/performance/per_h5CountFlow.action',
						search: false, 
