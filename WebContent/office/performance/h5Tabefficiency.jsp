...
<script type="text/javascript">
	...
	function tabPanel001Click(){
		document.getElementById('InfoIframe').src = "<%=path%>/performance/per_countEfficiency.action?startTime=${param.startTime}&endTime=${param.endTime}&templetId=${param.templetId}";
	}
	
	//图标标签
	function tabPanel002Click(){
		document.getElementById('InfoIframe').src = "<%=path%>/performance/per_chartEfficiency.action?startTime=${param.startTime}&endTime=${param.endTime}&templetId=${param.templetId}";
	}
	
	//打印
	function load(){
		var str = "<%=path%>/performance/pri_countEfficiency.action?templetId=${param.templetId}&startTime=${param.startTime}&endTime=${param.endTime}";
			...
			<tr>
				<td>
					<div id="tabContainer0_div" class="matrixComponentDiv" style="width: 100%; height: 100%;; position: relative;">
						...
						<iframe id="InfoIframe" style="width:100%;height:calc(100% - 35px);top:30px " frameborder="0" src="<%=path%>/performance/per_countEfficiency.action?startTime=${param.startTime}&endTime=${param.endTime}&templetId=${param.templetId}">
