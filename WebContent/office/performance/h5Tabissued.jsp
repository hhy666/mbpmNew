...
<title>流程已发已办标签页</title>
<script type="text/javascript">
	//表格标签
	function tabPanel001Click(){
		document.getElementById('InfoIframe').src = "<%=path%>/performance/per_countIssued.action?flowType=${param.flowType}&status=${param.status}&startTime=${param.startTime}&endTime=${param.endTime}";
	}
	
	//图标标签
	function tabPanel002Click(){
		document.getElementById('InfoIframe').src = "<%=path%>/performance/per_chartIssued.action?flowType=${param.flowType}&status=${param.status}&startTime=${param.startTime}&endTime=${param.endTime}";
	}
	
	//打印
	function load(){
		var str = "<%=path%>/performance/pri_countIssued.action?flowType=${param.flowType}&status=${param.status}&startTime=${param.startTime}&endTime=${param.endTime}";
			...
			<tr>
				<td>
					<div id="tabContainer0_div" class="matrixComponentDiv" style="width: 100%; height: 100%;; position: relative;">
						...
						<iframe id="InfoIframe" style="width:100%;height:calc(100% - 35px);top:30px " frameborder="0" src="<%=path%>/performance/per_countIssued.action?flowType=${param.flowType}&status=${param.status}&startTime=${param.startTime}&endTime=${param.endTime}">
