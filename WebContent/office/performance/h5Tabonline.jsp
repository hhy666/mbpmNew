...
<title>在线时间分析标签页</title>
<script type="text/javascript">
	//表格标签
	function tabPanel001Click(){
		document.getElementById('InfoIframe').src = "<%=path%>/performance/cha_countOnline.action?userId=${param.userId}&startTime=${param.startTime}&endTime=${param.endTime}&userName=${param.userName}&type=${param.type}";
	}
	
	<%-- //图标标签
	function tabPanel002Click(){
		document.getElementById('InfoIframe').src = "<%=path%>/performance/cha_chartOnline.action?userId=${param.userId}&startTime=${param.startTime}&endTime=${param.endTime}&userName=${param.userName}&type=${param.type}";
	} --%>
	
	//打印
	function load(){
		var str = "<%=path%>/performance/pri_countOnline.action?userId=${param.userId}&startTime=${param.startTime}&endTime=${param.endTime}&userName=${param.userName}&type=${param.type}";
			...
			<tr>
				<td>
					<div id="tabContainer0_div" class="matrixComponentDiv" style="width: 100%; height: 100%;; position: relative;">
						...
						<iframe id="InfoIframe" style="width:100%;height:calc(100% - 35px);top:30px " frameborder="0" src="<%=path%>/performance/cha_countOnline.action?userId=${param.userId}&startTime=${param.startTime}&endTime=${param.endTime}&userName=${param.userName}&type=${param.type}">
