...
<script type="text/javascript">
	...
	function tabPanel001Click(){
		document.getElementById('InfoIframe').src = "<%=path%>/performance/per_countOverTime.action?templetId=${param.templetId}&startTime=${param.startTime}&endTime=${param.endTime}&complete=${param.complete}&notFinish=${param.notFinish}&cutout=${param.cutout}";
	}
	
	//图标标签
	function tabPanel002Click(){
		document.getElementById('InfoIframe').src = "<%=path%>/performance/per_chartOverTime.action?templetId=${param.templetId}&startTime=${param.startTime}&endTime=${param.endTime}&complete=${param.complete}&notFinish=${param.notFinish}&cutout=${param.cutout}";
	}
	
	//打印
	function load(){
		var str = "<%=path%>/performance/pri_countOverTime.action?templetId=${param.templetId}&startTime=${param.startTime}&endTime=${param.endTime}&complete=${param.complete}&notFinish=${param.notFinish}&cutout=${param.cutout}";
			...
			<tr>
				<td>
					<div id="tabContainer0_div" class="matrixComponentDiv" style="width: 100%; height: 100%;; position: relative;">
						...
						<iframe id="InfoIframe" style="width:100%;height:calc(100% - 35px);top:30px " frameborder="0" src="<%=path%>/performance/per_countOverTime.action?templetId=${param.templetId}&startTime=${param.startTime}&endTime=${param.endTime}&complete=${param.complete}&notFinish=${param.notFinish}&cutout=${param.cutout}">
