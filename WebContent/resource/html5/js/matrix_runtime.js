    ...
    if (document.getElementById('is_mobile_request') && document.getElementById('is_mobile_request').value == 'true') {  //移动端
        ...
        */
    	layer.open({
            ...
            //maxmin: true, //开启最大化最小化按钮
            area: ['100%', '80%'],
            content: 'form/admin/querylist/advancedQuery.html?iframewindowid=DataGridCustomFilter&dataGridId=' + dataGridId
        });
    } else { //pc端
    	/*
        layer.open({
...
function onDataGridCustomFilterClose(data) {
    if (data != null) {
        ...
        var serviceQueryValue = data.serviceQueryValue;
        var dataGridId = data.dataGridId;
        
        var currentForm = document.getElementById('form0');
        
        var vituralQueryHidden = document.getElementById("" + dataGridId + "_mfilter");
        if (!vituralQueryHidden) {
            //添加高级查询隐藏域
            var advanceQueryHidden = document.createElement('input');
            advanceQueryHidden.type = "hidden";
            advanceQueryHidden.name = "" + dataGridId + "_mfilter";
            advanceQueryHidden.id = "" + dataGridId + "_mfilter";
            advanceQueryHidden.value = advanceQueryValue;      
            currentForm.appendChild(advanceQueryHidden);

        } else {
            Matrix.setFormItemValue("" + dataGridId + "_mfilter", advanceQueryValue);
        }
        
        var serviceQueryHidden = document.getElementById("" + dataGridId + "_servicequery");
        if (!serviceQueryHidden) {
        	//添加服务项查询隐藏域
        	var serviceQueryValueHidden = document.createElement('input');
            serviceQueryValueHidden.type = "hidden";
            serviceQueryValueHidden.name = "" + dataGridId + "_servicequery";
            serviceQueryValueHidden.id = "" + dataGridId + "_servicequery";
            serviceQueryValueHidden.value = serviceQueryValue;            
            currentForm.appendChild(serviceQueryValueHidden);
            
        } else {
        	Matrix.setFormItemValue("" + dataGridId + "_servicequery", serviceQueryValue);
        }
...
//弹出查询字段设置窗口
Matrix.openQueryListSet = function (optType, uuid, dataGridId, formId, queryListDefId) {
	...
	layer.open({	
		...
		shadeClose: false,
		area : [ '55%', '65%' ],
		content : webContextPath+'/form/admin/querylist/queryListSet.html?optType='+optType+'&uuid='+uuid+'&dataGridId='+dataGridId+'&formId='+formId+'&queryListDefId='+queryListDefId
	});
}

//查询字段设置窗口回调  选择的业务编码和机构编码
Matrix.onQueryListSetClose = function (data){
	var searchValue = data.searchValue;
	var dataGridId = data.dataGridId;
	var businessId = data.businessId;
	var selectedOrgId = data.selectedOrgId;
	
	var formId = data.formId;
	var queryListDefId = data.queryListDefId;
	var optType = data.optType;   //操作类型  add添加 update编辑
	var searchUuid = data.searchUuid;   //当前添加和修改的主键编码
	var searchName = data.searchName;   //当前添加和修改的名称
	
	var currentForm = document.getElementById('form0');
	 
	var businessIdHidden = document.getElementById("" + dataGridId + "_businessId");
    if (!businessIdHidden) {
    	var businessIdHidden = document.createElement('input');
    	businessIdHidden.type = "hidden";
    	businessIdHidden.name = dataGridId + "_businessId";
    	businessIdHidden.id = dataGridId + "_businessId";
    	businessIdHidden.value = businessId;
    	currentForm.appendChild(businessIdHidden);
    	 	
    }else{
    	Matrix.setFormItemValue("" + dataGridId + "_businessId", businessId);
    }
    
    var selectedOrgIdHidden = document.getElementById("" + dataGridId + "_selectedOrgId");
    if (!selectedOrgIdHidden) {
    	var businessIdHidden = document.createElement('input');
    	businessIdHidden.type = "hidden";
    	businessIdHidden.name = dataGridId + "_selectedOrgId";
    	businessIdHidden.id = dataGridId + "_selectedOrgId";
    	businessIdHidden.value = selectedOrgId;
    	currentForm.appendChild(businessIdHidden);
    	 	
    }else{
    	Matrix.setFormItemValue("" + dataGridId + "_selectedOrgId", selectedOrgId);
    }
    
    if(optType == 'add' && typeof(searchUuid)!="undefined" && typeof(searchName)!="undefined"){
    	if(document.getElementById('c_createQuerySet')){        		
    		var newLiHtml = '';
    		newLiHtml = newLiHtml + "<li id=\""+searchUuid+"\" class=\"item-pane\" style=\"height: 36px;line-height: 36px;font-size: 12px;padding-left: 10px;cursor:pointer;" +
                " \" onclick=\"Matrix.executeQueryInfo('"+searchUuid+"','"+dataGridId+"');\">"+searchName+"";
    		newLiHtml = newLiHtml + "<span class=\"item-icon\" style=\"display: none;float: right;padding-right: 10px;font-size:16px;color: #f56c6c;\" title=\"删除\" ";
    		newLiHtml = newLiHtml + "onclick=\"Matrix.deleteQueryInfo('"+searchUuid+"');\"><i class=\"fa fa-trash\"></i>";
    		newLiHtml = newLiHtml + "</span>";
    		newLiHtml = newLiHtml + "<span class=\"item-icon\" style=\"display: none;float: right;padding-right: 7px;padding-top: 1px;font-size:16px;\"";
    		newLiHtml = newLiHtml + "onclick=\"Matrix.openQueryListSet('update','"+searchUuid+"','"+dataGridId+"','"+formId+"','"+queryListDefId+"');\"><i class=\"fa fa-edit\" title=\"编辑\"></i>";
    		newLiHtml = newLiHtml + "</span>";
    		newLiHtml = newLiHtml + "</li>";
    		
    		$('#c_createQuerySet').before(newLiHtml);
    	}
    }
}

//点击执行查询信息
Matrix.executeQueryInfo = function (uuid, dataGridId){
	var url = webContextPath+'/queryListSet/getSelectedFieldInfo';
	var synJson = {};
	synJson.uuid = uuid;
	Matrix.sendRequest(url,synJson,function(data){
		if(data.data){
			...
			var json = data.data;
			var data = {};
    		data.searchValue = JSON.stringify(json.selectField);  		 
    		data.dataGridId = dataGridId;
    		data.businessId = json.businessId;
     		data.selectedOrgId = json.selectedOrgId;
     	
    		Matrix.queryDataGrid4Set(dataGridId, data);
    		Matrix.onQueryListSetClose(data);
		}
    });
}

//删除查询字段设置
Matrix.deleteQueryInfo = function (uuid){
	var e = window.event || arguments.callee.caller.arguments[0];
	e.stopPropagation();  //阻止向上冒泡
	...
	layer.confirm("确认删除？", {
    	...
    	var url = webContextPath+'/queryListSet/deleteSearchInfo';
    	var synJson = {};
    	synJson.uuid = uuid;
    	Matrix.sendRequest(url,synJson,function(data){
    		if(data.data==true){
    ...
    if (id == "DataGrid001") {
        ...
        if (com != null) {
            if (com.tagName == 'IFRAME') {
                com.setContentsURL = function (src) {
                    com.src = src;
                };
                return com;
            }
...
Matrix.getFormItemValue = function (formItemId) {
    ...
    if (com) {
        if (com.type == "checkbox") {
            // 复选框
            return $("input:checkbox[name='"+formItemId+"']").is(":checked")
        } else {
            return com.value;
        }
    } else {
        const checkBoxDoms = $("input:checkbox[groupid='"+formItemId+"']")
        if (typeof checkBoxDoms != "undefined" && checkBoxDoms != null && checkBoxDoms.length>0) {
            // 复选框组
            const checkedGroup = $("input:checkbox[groupid='serviceItem_1_checkboxgroup001']:checked")
            const checkedValArr = new Array(checkedGroup.length)
            for (let i = 0; i < checkedGroup.length; i++) {
                checkedValArr[i] = $("input:checkbox[groupid='serviceItem_1_checkboxgroup001']:checked:eq("+i+")").val()
            }

            return checkedValArr.toString()
        }
        const radioDoms = $("input:radio[name='"+formItemId+"']")
        if (typeof radioDoms != "undefined" && radioDoms != null && radioDoms.length>0){
            // 单选按钮组
            return $("input:radio[name='"+formItemId+"']:checked").val()
        }
    }
    return null;
};
...
Matrix.showFormItem = function (formItemId) {
    Matrix.setFormItemDisplay(formItemId + '_FID', true);
    Matrix.setFormItemDisplay(formItemId, true);

}

Matrix.hideFormItem = function (formItemId) {
    Matrix.setFormItemDisplay(formItemId + '_FID', false);
    Matrix.setFormItemDisplay(formItemId, false);
}
...
Matrix.validateForm = function (formId) {
    ...
    if (validInfo) { //该表单有联合校验
        ...
        Matrix.sendSyncRequest(url, newData, function (data) {
            if ("true" == data.data) {
            } else {
                Matrix.warn(data.data);
                result = false;
            }
        });
...
if ($.validator) {
    $.validator.addMethod('isRequired', function (value, element, params) {
        return fileUploadRequired(value, element);
    }, '不能为空');
}
        ...
        if (typeof (image) == "undefined" || image == "false") {
            ...
            if (count == 0) { //如果当前没上传，但同一附件已有上传的则也通过
                ...
                if (containerDiv == null) {
                    containerId = fileInputId.substring(0, fileInputId.indexOf("_file", 0)) + "_file_InputDIV";
...
Matrix.closeLayerWindow = function (data, operationType) {
    Matrix.closeWindow(data, operationType)
}
...
Matrix.closeWindow = function (data, operationType) {
    ...
    if (typeof MatrixDialog != "undefined" && MatrixDialog.closeAndCallBack(iframewindowid, data, operationType)) {
        return;
    }
    var closeFunction;
...
function deleteCurRow(){
  var _dataGridId = Matrix.getFormItemValue('clickDataGrid');
  var _clickRowIndex = Matrix.getFormItemValue('clickRowIndex');
	Matrix.deleteRepeatRowForTable(_clickTd,null,_dataGridId,_clickRowIndex);
}
...
Matrix.deleteRepeatRow= function deleteRow(divId,datalistId,currentindex){
  ...
  Matrix.hideMask2();
	...
	//设置选中行信息
	function setSelectRow(obj,dataListId,rowIndex){
	  _clickTd = obj;
	...
	//显示重复节增加行和删除行的按钮
	function showPlusAndReduceButton(obj,dataListId,rowIndex,canAdd, canDel){
	  _clickTd = obj;
...
//增加行
Matrix.addRepeatRowForTable =  function addRowForTable(obj,divId,datalistId,currentindex,gridId,newRowData){
   Matrix.showMask2();
...
//			setTimeout("$('input').iCheck({ checkboxClass: 'icheckbox_square-blue', increaseArea: '20%'  }); ", 100 );
		...
		if(newRowData){
			...
			var  rowIndex = addindex;
			var gridFields = eval(gridId+"DFields");
			for(var i=0; i<gridFields.length; ++i){
			   var json = gridFields[i];
			   var pId = json.id;
			   var pName = json.propName;

			   for(var key in newRowData){ 
					if(key == pId){
						if(document.getElementById(key+"_"+rowIndex)){
								Matrix.setFormItemValue(key+"_"+rowIndex, newRowData[key]);
						}
						if(document.getElementById(key+"_"+rowIndex+"_lbl")){
								document.getElementById(key+"_"+rowIndex+"_lbl").innerHTML = newRowData[key];
						}
					}else if(key == pName){
						if(document.getElementById(pId+"_"+rowIndex)){
							Matrix.setFormItemValue(pId+"_"+rowIndex, newRowData[key]);
						}
						if(document.getElementById(pId+"_"+rowIndex+"_lbl")){
							document.getElementById(pId+"_"+rowIndex+"_lbl").innerHTML = newRowData[key];
						}
					}
...
Matrix.addRepeatRowsWithData =  function addRowForTable(obj,divId,datalistId,currentindex,data){
 ...
 Matrix.addRepeatRowForTable(obj,null,datalistId,addindex,gridId, data);
...
//获取重复表
Matrix.getRepeatTableData =  function addRowForTable(gridId){
   Matrix.showMask2();
  ...
  var clickDataGrid = Matrix.getFormItemValue('clickDataGrid');  //选中行所在的重复表id
   ...
   if(rowsArr.length == 0){
      return data;
	     ...
	     var c = rowsArr;
	     for(var i=0;i<c.length;i++){
	         var index = parseInt(c[i]);
			 var rowData = Matrix.getRepeatRowData(gridId, index);
		     data.push(rowData);
	     }
   }
...
Matrix.hideComponent = function (componentId) {
    if (componentId != null) {
        com = document.getElementById(componentId + 'Item_FID_div');
        if (com) {
        } else {
            com = document.getElementById(componentId + '_div');
        }
        if (com != null) {
            com.style.display = 'none';
        }
...
Matrix.conditionFormatter = function (value, options, row, index) {
    ...
    if (options != null && options.length > 0) {
        for (var i = 0; i < options.length; i++) {
			...
			var adResult = eval(op._adCondition);
			if(result && adResult && op._value!=null){
...
function mSelectUsers(clientId, id) {
    if (document.getElementById('is_mobile_request') && document.getElementById('is_mobile_request').value == 'true') {
        var pageii = layer.open({
            ...
            shadeClose: false, //开启遮罩关闭
            area: ['100%', '100%'],
            content: webContextPath +'/office/html5/select/MultiSelectPerson.jsp?iframewindowid=SelectUsersDialog&clientId=' + clientId + '&id=' + id
        });
    } else {
        /*var pageii = layer.open({
				id:clientId,
...
function mSelectDeps(clientId, id) {
    if (document.getElementById('is_mobile_request') && document.getElementById('is_mobile_request').value == 'true') {
        var pageii = layer.open({
            ...
            shadeClose: false, //开启遮罩关闭
            area: ['100%', '100%'],
            content: webContextPath +'office/html5/select/MultiSelectDep.jsp?iframewindowid=SelectDepsDialog&clientId=' + clientId + '&id=' + id
        });
    } else {
        /*var pageii = layer.open({
            id:clientId,
...
isc.say = function (message) {
    ...
    if (typeof (MatrixLang) != "undefined") {
        Matrix.say(MatrixLang.geti18nInfo("提示信息"), MatrixLang.geti18nInfo(message));
    } else {
...
isc.warn = function (message) {
    ...
    if (typeof (MatrixLang) != "undefined") {
        Matrix.warn(MatrixLang.geti18nInfo("警告信息"), MatrixLang.geti18nInfo(message));
    } else {
        Matrix.say("警告信息", message);
    }
}
...
isc.confirm = function (message, callback, properties) {
    ...
    if (properties != null && properties != "") {
        //layer.confirm( MatrixLang.geti18nInfo(message),{  time: 0,properties,yes:function (index) {layer.close(index);callback()},no:function (index){Matrix.hideMask();return false;}});
    } else {
        if (typeof (MatrixLang) != "undefined") {
            layer.confirm(MatrixLang.geti18nInfo(message), {
                time: 0, btn: [MatrixLang.geti18nInfo('确定'), MatrixLang.geti18nInfo('取消')], yes: function (index) {
                    layer.close(index);
                    callback()
                }, no: function (index) {
                    Matrix.hideMask2();
...
Matrix.say = function (message, title) {
    ...
    if (typeof (MatrixLang) != "undefined") {
        toastr.info(MatrixLang.geti18nInfo(message), MatrixLang.geti18nInfo(title));
    } else {
...
Matrix.warn = function (message, title) {
    ...
    if (typeof (MatrixLang) != "undefined") {
        toastr.warning(MatrixLang.geti18nInfo(message), MatrixLang.geti18nInfo(title));
    } else {
        toastr.warning(message, title);
    }
}
...
Matrix.success = function (message, title) {
    ...
    if (typeof (MatrixLang) != "undefined") {
        toastr.success(MatrixLang.geti18nInfo(message), MatrixLang.geti18nInfo(title));
    } else {
...
Matrix.error = function (message, title) {
    ...
    if (typeof (MatrixLang) != "undefined") {
        toastr.error(MatrixLang.geti18nInfo(message), MatrixLang.geti18nInfo(title));
    } else {
...
Matrix.deleteTreeNode = function (target, message, deleteCallback) {
    ...
    layer.confirm(message, {
        time: 0, yes: function (index) {
            layer.close(index);
            postRemoveTreeNode(target, deleteCallback)
        }, no: function (index) {
            Matrix.hideMask2();
            return false;
        }
...
Matrix.addRepeatLastRow=function(gridId,newRowData){
	addRepeatLastRow(gridId,newRowData);
}

Matrix.addRowsOfRepeatGrid=function(gridId,newRowData){
	addRepeatLastRow(gridId,newRowData);
}


function addRepeatLastRow(gridId,newRowData){  //重复表添加一行
...
//     var addindex = parseInt(max)+1+"";
   ...
   Matrix.addRepeatRowForTable(obj,null,datalistId,addindex,gridId,newRowData);
}

//修改重复表指定行数据
Matrix.updateRepeatRow=function(gridId, rowIndex, data){
	if(gridId == null || rowIndex <0 || data == null)
		return;
	...
	for(var i=0; i<gridFields.length; ++i){
	   ...
	   var pName = json.propName;
		...
		if(document.getElementById(pId+"_"+rowIndex)){
			if(data[pId]){
				Matrix.setFormItemValue(pId+"_"+rowIndex, data[pId]);
			}else{
				Matrix.setFormItemValue(pId+"_"+rowIndex, '');
			}
		}
		if(document.getElementById(pId+"_"+rowIndex+"_lbl")){
			if(data[pId]){
				document.getElementById(pId+"_"+rowIndex+"_lbl").innerHTML = data[pId];
			}else{
				document.getElementById(pId+"_"+rowIndex+"_lbl").innerHTML = '';
			}
        }

		if(pId != pName){
			if(document.getElementById(pName+"_"+rowIndex)){
				if(data[pName]){
					Matrix.setFormItemValue(pName+"_"+rowIndex, data[pName]);
				}else{
					Matrix.setFormItemValue(pName+"_"+rowIndex, '');
				}
			}
			if(document.getElementById(pName+"_"+rowIndex+"_lbl")){
				if(data[pName]){
					document.getElementById(pName+"_"+rowIndex+"_lbl").innerHTML = data[pName];
				}else{
					document.getElementById(pName+"_"+rowIndex+"_lbl").innerHTML = '';
				}	
			}
		}
...
function getMaxRowNumberOfRepeat(gridId){
	 var datalistId = gridId + "D";
   var obj = null;
//   var obj = _clickTd;  //传所在的td element
...
Matrix.deleteRepeatCurRow=function(gridId){ //删除重复表中的当前行

	deleteRepeatCurRow(gridId);
}
...
function deleteRepeatCurRow(gridId){
     ...
     if(clickDataGrid != datalistId){
     	Matrix.say("matrix.noDataGridDeleteSelect");
     	return;
     }
...
Matrix.getCurRow=function(gridId){ //获取当前行json对象(属性为客户端编码)
	var datalistId = gridId + "D";
    var obj = _clickTd;  //传所在的td element
	...
	for(var i=0; i<gridFields.length; ++i){
	   var json = gridFields[i];
	   var pId = json.id;
	   var pId2 = pId + "_" + rowIndex;
	   var pIdLbl = pId2+"_lbl";
	   var pName = json.propName;
	   var pName2 = pName + "_" + rowIndex;
	   var pNameLbl = pName2 +"_lbl";
	   if(Matrix.getFormItemValue(pId2) != null || Matrix.getFormItemValue(pIdLbl) != null){ //id存在
		   if(Matrix.getFormItemValue(pId2) != null){
			  rowData[pId] = Matrix.getFormItemValue(pId2);
			  if(pId != pName){
				  rowData[pName] = Matrix.getFormItemValue(pId2);
			  }	
		   }else{
			   if(document.getElementById(pIdLbl) != null){
					rowData[pId] = document.getElementById(pIdLbl).innerHTML;
				    if(pId != pName){
					    rowData[pName] = document.getElementById(pIdLbl).innerHTML;
					}
			  }else{
					rowData[pId] = '';
				    if(pId != pName){
	   ...
	   }else if(pId != pName && Matrix.getFormItemValue(pName2) != null || Matrix.getFormItemValue(pNameLbl) != null){ //id不存在，属性存在
		   if(Matrix.getFormItemValue(pName2) != null){
			  rowData[pId] = Matrix.getFormItemValue(pName2);
			  rowData[pName] = Matrix.getFormItemValue(pName2);
		   }else{
			   if(document.getElementById(pNameLbl) != null){
					rowData[pId] = document.getElementById(pNameLbl).innerHTML;
					rowData[pName] = document.getElementById(pNameLbl).innerHTML;
			  }else{
					rowData[pId] = '';
				    rowData[pName] = '';
			  }	
...
Matrix.getRepeatRowData=function(gridId, rowIndex){ //获取当前行json对象(属性为客户端编码)
	var datalistId = gridId + "D";
    var obj = _clickTd;  //传所在的td element
...
Matrix.deleteAllRowsOfRepeat=function(gridId,propId){ //删除重复表中的所有行

	deleteAllRowsOfRepeatGrid(gridId,propId);
}

function deleteAllRowsOfRepeatGrid(gridId,propId){
	   var datalistId = gridId + "D";
		   ...
		   var rowsArr = rowsvar.split(",");
    		for (var i = 0; i < rowsArr.length; i++) {
    			var rowIndex = rowsArr[i];
              	if(rowIndex>-1) {
              	    var checkEle = document.getElementById(propId+"_"+rowIndex); 
					var value = checkEle.value;
//					if($("#"+gridId+'_trCheckBox_'+rowIndex).is(':checked')){
					//if(value=='1'||value=='true'){
					    var obj = checkEle.parentNode.parentNode;
						Matrix.deleteRepeatRowForTable(obj,null,datalistId,rowIndex);
						deletedRows = true;
//       				}
              	}		
...
function moveUpSelectedRow(gridId){ 
     ...
     if(selectedCount == 0){
     	Matrix.say("matrix.moveUp");
     	return;
     }

     if(selectedCount > 1){
     	Matrix.say("matrix.moveUpSelectOne");
     	return;
     }
    ...
    if(curIndex == 0){
     	Matrix.say("matrix.oneRowNoMoveUp");
     	return;
    }
...
function moveDownSelectedRow(gridId){ 
	 ...
	 if(selectedCount == 0){
     	Matrix.say("matrix.moveDown");
     	return;
     }

     if(selectedCount > 1){
     	Matrix.say("matrix.moveDownSelectOne");
     	return;
     }
   ...
   var curIndex = 0;   //当前行索引
     ...
     if(rowsvar!=null&&rowsvar!=""){
	    ...
	    if(curIndex == (rowsArr.length-1)){
	        Matrix.say('matrix.lastRowNotMove');
	        return;
	    }
...
Matrix.exportEditList = function (listId) {
    ...
    var url = webContextPath + '/matrix.rform?matrix_send_request=true&matrix_form_tid=' + document.getElementById("matrix_form_tid").value + '&formId=' + formCode;
    ...
    html += '</form>';
    $('body').append(html);
    var formArr = $("#form0").serializeArray();
    //$("#" + tableId).clone(true).appendTo("#" + formId);
    formArr.forEach(function (item) {
        $("#" + formId).append("<input type='hidden' name='" + item.name + "' value='" + item.value + "' />");
    })
 ...
 * @author zsd
 */
Matrix.addPostContabs = function (parJson, keys, values) {
    var options = {
        title: "",
        href: ""
    };
    var ops = $.extend(options, parJson);
    ...
    window.top.$(".J_menuTab").each(function () {
        if (window.top.$(this).data("id") == o) {
            if (!window.top.$(this).hasClass("active")) {
                window.top.$(this).addClass("active").siblings(".J_menuTab").removeClass("active");
                //g(this);
                window.top.$(".J_mainContent .J_iframe").each(function () {
                    if (window.top.$(this).data("id") == o) {
                        window.top.$(this).show().siblings(".J_iframe").hide();
                        return false
                    }
                })
            }
    ...
    if (k) {
        ...
        var n = '<iframe class="J_iframe" name="iframe' + m + '" width="100%" height="100%" src="" frameborder="0" data-id="' + o + '" seamless></iframe>';
        window.top.$(".J_mainContent").find("iframe.J_iframe").hide().parents(".J_mainContent").append(n);
        window.top.$(".J_menuTabs .page-tabs-content").append(p);
        //g($(".J_menuTab.active"))
        Matrix.openPostWindow(ops.href, keys, values, "iframe" + m);
        return true;
    }
    return false
};
 ...
 * @author zsd
 */
Matrix.refreshTab = function (J_iframe_id) {
    var target = $('.J_iframe[data-id="' + J_iframe_id + '"]');
    var url = target.attr('src');
    target.attr('src', url).load();
}
/**
 * 循环设置form隐藏参数，并执行 target= newwindow, _blank 在新窗口中打开。 _self 默认。在相同的框架中打开。
 * _parent 在父框架集中打开。 _top 在整个窗口中打开。 framename 在指定的框架中打开。
 *
 * @author liupengfei
 */
Matrix.openPostWindow = function (url, keys, values, target) {
    var tempForm = document.createElement("form");
    tempForm.method = "post";
    tempForm.action = url;
    if (Matrix.isNotEmpty(target)) {
 ...
 * @author zsd
 */
Matrix.closeMenuByDataId = function (dataId) {
    if (Matrix.isNotEmpty(dataId)) {
        parent.parent.$('.J_menuTab ').each(function () {
 ...
 * @author zsd
 */
Matrix.isNotEmpty = function (obj) {
 ...
 * @author zsd
 */
Matrix.addContabs = function (parJson) {
    var options = {
        title: "",
        href: ""
    };
 ...
 *选择流转处理人员
 */
Matrix.openSelectPotentialOwners = function (clientId, id) {
    var matrixTransitions = Matrix.getFormItemValue("matrix_transitions");
    ...
    if (matrixTransitions == null || matrixTransitions == "") {
        Matrix.warn("请选择下一步后选择执行人");
        return;
    }
    var pageii = layer.open({
        id: clientId,
        ...
        shadeClose: false, //开启遮罩关闭
        area: ['70%', '90%'],
        content: 'office/html5/select/MultiSelectPotentialOwners.jsp?iframewindowid=SelectUsersDialog&matrixTransitions=' + matrixTransitions + '&aiid=' + aiid + '&pdid=' + pdid + '&clientId=' + clientId + '&id=' + id
    });
}

Matrix.queryDataGrid4Set = function (id, jsonArr) {
    var newData = {};
    Matrix.setFormItemValue("displayFieldDef_Str", jsonArr.searchValue)
    newData["displayFieldDef_Str"] = jsonArr.searchValue;
    newData["refresh_options_data_grid_id"] = id;
    var url = webContextPath + '/matrix.rform?mHtml5Flag=true&matrix_send_request=true&matrix_form_tid=' + document.getElementById("matrix_form_tid").value;
    Matrix.sendRequest(url, newData, function (data) {
        if (data.data != null && data.data != "") {
            debugger;
            var synJson = eval('(' + data.data + ')');
            $('#' + id + '_table').bootstrapTable('refreshOptions', synJson);
        }
    });
}